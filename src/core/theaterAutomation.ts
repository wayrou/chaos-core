import {
  AnnexFrameType,
  AnnexInstance,
  AutomationSignalSnapshot,
  AutomationSignalValue,
  PartitionInstance,
  TheaterAutomationModuleRuntime,
  TheaterAutomationRuntimeState,
  TheaterNetworkState,
  TheaterRoom,
} from "./types";

const EMPTY_SIGNAL: AutomationSignalValue = { kind: "empty", value: 0 };

export type TheaterNodeKind = "room" | "annex";

export type TheaterResolvedNode = {
  id: string;
  kind: TheaterNodeKind;
  label: string;
  position: { x: number; y: number };
  size: { width: number; height: number };
  room: TheaterRoom | null;
  annex: AnnexInstance | null;
  rootRoom: TheaterRoom | null;
};

const ANNEX_FRAME_LABELS: Record<AnnexFrameType, string> = {
  lightweight_annex: "Light",
  standard_annex: "Standard",
  heavy_annex: "Heavy",
};

function cloneSignal(signal?: AutomationSignalValue | null): AutomationSignalValue {
  if (!signal) {
    return EMPTY_SIGNAL;
  }
  return { ...signal };
...
function cloneRuntime(runtime?: TheaterAutomationModuleRuntime | null): TheaterAutomationModuleRuntime {
  return {
...
export function createEmptyTheaterAutomationState(): TheaterAutomationRuntimeState {
  return {
    moduleInstancesById: {},
    moduleRuntimeById: {},
...
export function normalizeTheaterAutomationState(automation?: Partial<TheaterAutomationRuntimeState> | null): TheaterAutomationRuntimeState {
  return {
    moduleInstancesById: Object.fromEntries(
      Object.entries(automation?.moduleInstancesById ?? {}).map(([instanceId, instance]) => [
        instanceId,
        {
          ...instance,
    ...
    moduleRuntimeById: Object.fromEntries(
      Object.entries(automation?.moduleRuntimeById ?? {}).map(([instanceId, runtime]) => [
        instanceId,
...
export function normalizeTheaterAnnexes(annexes?: Record<string, AnnexInstance> | null): Record<string, AnnexInstance> {
  return Object.fromEntries(
    Object.entries(annexes ?? {}).map(([annexId, annex]) => [
      annexId,
      {
        ...annex,
...
export function normalizeTheaterPartitions(partitions?: Record<string, PartitionInstance> | null): Record<string, PartitionInstance> {
  return Object.fromEntries(
    Object.entries(partitions ?? {}).map(([edgeId, partition]) => [
      edgeId,
      {
        ...partition,
...
export function getTheaterEdgeId(nodeAId: string, nodeBId: string): string {
  return [nodeAId, nodeBId].sort().join("__");
}

export function getTheaterSelectedNodeId(theater: TheaterNetworkState): string {
  return theater.selectedNodeId ?? theater.selectedRoomId;
}

export function getTheaterCurrentNodeId(theater: TheaterNetworkState): string {
  return theater.currentNodeId ?? theater.currentRoomId;
}

export function getAnnexChildIds(theater: TheaterNetworkState, parentNodeId: string): string[] {
  return Object.values(theater.annexesById ?? {})
    .filter((annex) => annex.parentNodeId === parentNodeId)
    .map((annex) => annex.annexId);
}

export function getTheaterRootRoomIdForNode(theater: TheaterNetworkState, nodeId: string | null | undefined): string | null {
  if (!nodeId) {
    return null;
  }
...
export function getTheaterRootRoomForNode(theater: TheaterNetworkState, nodeId: string | null | undefined): TheaterRoom | null {
  const roomId = getTheaterRootRoomIdForNode(theater, nodeId);
  return roomId ? theater.rooms[roomId] ?? null : null;
}

export function resolveTheaterNode(theater: TheaterNetworkState, nodeId: string | null | undefined): TheaterResolvedNode | null {
  if (!nodeId) {
    return null;
  }
...
export function isTheaterNodeVisible(theater: TheaterNetworkState, nodeId: string): boolean {
...
export function isTheaterNodeSecured(theater: TheaterNetworkState, nodeId: string): boolean {
  const resolved = resolveTheaterNode(theater, nodeId);
  if (!resolved) {
    return false;
  }
...
export function getTheaterNodeAdjacency(theater: TheaterNetworkState, nodeId: string): string[] {
  const room = theater.rooms[nodeId];
  if (room) {
...
export function isBlastDoorClosed(theater: TheaterNetworkState, nodeAId: string, nodeBId: string): boolean {
  return theater.partitionsByEdgeId?.[getTheaterEdgeId(nodeAId, nodeBId)]?.state === "closed";
}

function isRoomToRoomEdgePowered(theater: TheaterNetworkState, nodeAId: string, nodeBId: string): boolean {
  ...
  const requirement = Math.max(
    Math.max(0, roomA.powerGateWatts?.[roomB.id] ?? 0),
    Math.max(0, roomB.powerGateWatts?.[roomA.id] ?? 0),
  );
  if (requirement <= 0) {
    return true;
  }
...
export function canTraverseTheaterEdge(theater: TheaterNetworkState, fromNodeId: string, toNodeId: string): boolean {
...
export function getTheaterNodeMoveTickCost(theater: TheaterNetworkState, nodeId: string): number {
  const room = theater.rooms[nodeId];
  if (!room) {
    return 1;
  }
...
export function findTheaterNodeRoute(
  theater: TheaterNetworkState,
  fromNodeId: string,
  toNodeId: string,
  options?: {
    allowUnsecuredDestination?: boolean;
    allowUnknownDestination?: boolean;
    ignoreClosedPartitions?: boolean;
  },
): string[] | null {
  ...
  const queue: Array<{ nodeId: string; cost: number }> = [{ nodeId: fromNodeId, cost: 0 }];
...
function findRootRoomRoute(theater: TheaterNetworkState, fromRoomId: string, toRoomId: string): string[] | null {
  ...
  const path: string[] = [];
  let cursor: string | null = toRoomId;
  while (cursor) {
    path.push(cursor);
    cursor = previous.get(cursor) ?? null;
  }
...
export function canAutomationReachTarget(
  theater: TheaterNetworkState,
  sourceNodeId: string,
  targetNodeId: string,
  requiredBw: number,
...
export function applyAnnexInheritedNetwork(theater: TheaterNetworkState): TheaterNetworkState {
...
function getSignalNumber(signal?: AutomationSignalValue | null): number {
  if (!signal) {
    return 0;
  }
...
function getSignalBoolean(signal?: AutomationSignalValue | null): boolean {
  if (!signal) {
    return false;
  }
...
function asBooleanSignal(value: boolean): AutomationSignalValue {
  return { kind: "boolean", value };
}

function asNumberSignal(value: number): AutomationSignalValue {
  return { kind: "number", value: Number.isFinite(value) ? value : 0 };
}

function addRoomOverlay(theater: TheaterNetworkState, roomId: string, type: "power" | "supply" | "comms", amount: number): void {
...
function getRoomEffectiveValue(theater: TheaterNetworkState, roomId: string, type: "power" | "supply" | "comms"): number {
...
function getModuleRuntime(theater: TheaterNetworkState, moduleId: string): TheaterAutomationModuleRuntime {
  const automation = theater.automation!;
...
function readModuleInputs(theater: TheaterNetworkState, moduleId: string): AutomationSignalValue[] {
...
function spendStoredAmount(theater: TheaterNetworkState, hostNodeId: string, amount: number, preferredModuleTypes: string[]): { released: number; moduleId: string | null } {
  const modules = Object.values(theater.automation!.moduleInstancesById).filter((module) =>
    module.installedNodeId === hostNodeId && preferredModuleTypes.includes(module.moduleType),
  );
...
function buildSignalSnapshot(moduleId: string, label: string, output: AutomationSignalValue, active: boolean): AutomationSignalSnapshot {
  return {
    moduleId,
    label,
...
function applyRoomFlowOverlays(theater: TheaterNetworkState): TheaterNetworkState {
  const automation = theater.automation!;
...
export function getAutomatedTurretCountForRoom(theater: TheaterNetworkState, roomId: string): number {
...
export function evaluateTheaterAutomation(theater: TheaterNetworkState): TheaterNetworkState {
  ...
  const evaluateBatch = (batch: typeof modules) => {
    batch.forEach((module) => {
      ...
      switch (module.moduleType) {
        case "threat_sensor": {
          ...
          if (active) console.log("[AUTOMATION] sensor fired", module.moduleType, module.instanceId, threatScore);
          break;
        }
        case "power_sensor": {
          const monitoredNodeId = module.configuration.monitorTarget?.nodeId ?? module.installedNodeId;
          ...
          active = power > 0;
          if (active) console.log("[AUTOMATION] sensor fired", module.moduleType, module.instanceId, power);
          break;
        }
        case "bandwidth_sensor": {
          ...
          active = bw > 0;
          if (active) console.log("[AUTOMATION] sensor fired", module.moduleType, module.instanceId, bw);
          break;
        }
        case "threshold_switch": {
          ...
          if (active) console.log("[AUTOMATION] logic resolved true", module.moduleType, module.instanceId, inputValue, comparison, threshold);
          break;
        }
        case "delay_timer": {
          ...
          output = asBooleanSignal(active);
          if (active) console.log("[AUTOMATION] logic resolved true", module.moduleType, module.instanceId, runtime.timerTicks);
          break;
        }
        case "and_gate": {
          active = inputs.length > 0 && inputs.every((signal) => getSignalBoolean(signal));
          output = asBooleanSignal(active);
          if (active) console.log("[AUTOMATION] logic resolved true", module.moduleType, module.instanceId);
          break;
        }
        case "or_gate": {
        ...
        case "bandwidth_router": {
          ...
          if (hostRoomId && targetRoomId && shouldFire && reachable) {
            ...
            if (active) console.log("[AUTOMATION] actuator executed", module.moduleType, module.instanceId, `${hostRoomId}->${targetRoomId}`, transferred);
          } else {
            active = false;
          }
          output = asBooleanSignal(active);
        ...
        case "cache_release": {
          ...
          active = released > 0;
          if (active) console.log("[AUTOMATION] actuator executed", module.moduleType, module.instanceId, targetRoomId, released);
          output = asBooleanSignal(active);
          break;
        }
        ...
        case "turret_controller": {
          ...
          if (hostRoomId && shouldFire && (theater.rooms[hostRoomId]?.fortificationPips.turret ?? 0) > 0) {
            theater.activeThreats = theater.activeThreats.map((threat) => (
              threat.active && threat.currentRoomId === hostRoomId
      ...
      theater.automation!.activeSignalSnapshots.push(
        buildSignalSnapshot(module.instanceId, module.moduleType, runtime.lastOutput, runtime.active),
      );
    });
  };
