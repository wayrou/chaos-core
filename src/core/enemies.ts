...
type PlaceholderEnemySpec = {
  name: string;
  role: EnemyDefinition["role"];
  rarity: EnemyDefinition["rarity"];
  minFloorInRegion?: 1 | 2 | 3;
  weight?: number;
};

const REGION_ENEMY_FACTIONS: Record<CampaignRegionId, EnemyFactionId[]> = {
...
const FACTION_REGION_MAP: Record<EnemyFactionId, CampaignRegionId[]> = {
  maw: ["overgrowth"],
  rivet: ["industrial"],
  anchor: ["flooded"],
  null: ["chaos"],
...
const PLACEHOLDER_ENEMY_ROSTERS: Record<EnemyFactionId, PlaceholderEnemySpec[]> = {
  maw: [
...
const ROLE_BASE_STATS: Record<EnemyDefinition["role"], EnemyDefinition["baseStats"]> = {
...
const RARITY_STAT_MODIFIERS: Record<EnemyDefinition["rarity"], Partial<EnemyDefinition["baseStats"]>> = {
...
const RARITY_DEFAULT_WEIGHTS: Record<EnemyDefinition["rarity"], number> = {
  common: 10,
  uncommon: 7,
  rare: 4,
  elite: 2,
...
function toEnemyId(name: string): string {
  return name
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "_")
    .replace(/^_+|_+$/g, "");
...
function getPlaceholderDeck(role: EnemyDefinition["role"]): string[] {
  switch (role) {
    case "tank":
...
function buildPlaceholderBaseStats(
  role: EnemyDefinition["role"],
  rarity: EnemyDefinition["rarity"],
): EnemyDefinition["baseStats"] {
...
function getDefaultMinFloorInRegion(rarity: EnemyDefinition["rarity"]): 1 | 2 | 3 {
  if (rarity === "common") {
    return 1;
  }
  if (rarity === "uncommon") {
...
function buildPlaceholderEnemyDefinition(
  factionId: EnemyFactionId,
  spec: PlaceholderEnemySpec,
): EnemyDefinition {
...
function buildPlaceholderEnemyDatabase(): Record<string, EnemyDefinition> {
  const database: Record<string, EnemyDefinition> = {};
  (Object.keys(PLACEHOLDER_ENEMY_ROSTERS) as EnemyFactionId[]).forEach((factionId) => {
...
function parseOpsAtlasFloorOrdinal(operationId: string): number | null {
...
export function getCampaignRegionEnemyPool(floorOrdinal: number): EnemyPool {
...
export function pickEnemyIdsForCampaignFloor(
  floorOrdinal: number,
  count: number,
