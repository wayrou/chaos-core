...
export const FOUNDRY_MODULE_UNLOCK_ORDER: AutomationModuleType[] = [
  "threat_sensor",
  "power_sensor",
  "bandwidth_sensor",
  "loom_terminal",
...
export const FOUNDRY_PARTITION_UNLOCK_ORDER: PartitionType[] = [
  "blast_door",
];

export const FOUNDRY_MODULE_DEFINITIONS: Record<AutomationModuleType, ModuleDefinition> = {
  threat_sensor: {
    id: "threat_sensor",
...
export const FOUNDRY_PARTITION_DEFINITIONS: Record<PartitionType, PartitionDefinition> = {
  blast_door: {
    id: "blast_door",
...
export function getOrderedFoundryModuleTypes(): AutomationModuleType[] {
  return [...FOUNDRY_MODULE_UNLOCK_ORDER];
}

export function getOrderedFoundryPartitionTypes(): PartitionType[] {
  return [...FOUNDRY_PARTITION_UNLOCK_ORDER];
}

export function createDefaultFoundryState(): FoundryUnlockState {
  return {
    unlockedModuleTypes: [],
...
export function normalizeFoundryUnlockState(foundry?: Partial<FoundryUnlockState> | null): FoundryUnlockState {
  const moduleSet = new Set<AutomationModuleType>();
  ...
  const partitionSet = new Set<PartitionType>();
...
export function withNormalizedFoundryState<T extends GameState>(state: T): T {
...
export function getFoundryUnlockState(state: GameState): FoundryUnlockState {
  return normalizeFoundryUnlockState(state.foundry);
}

export function isModuleTypeUnlocked(state: GameState, moduleType: AutomationModuleType): boolean {
  return getFoundryUnlockState(state).unlockedModuleTypes.includes(moduleType);
}

export function isPartitionTypeUnlocked(state: GameState, partitionType: PartitionType): boolean {
  return getFoundryUnlockState(state).unlockedPartitionTypes.includes(partitionType);
}

export function unlockFoundryModuleTypeInState(state: GameState, moduleType: AutomationModuleType): FoundryUnlockResult {
...
export function unlockFoundryPartitionTypeInState(state: GameState, partitionType: PartitionType): FoundryUnlockResult {
  const normalizedState = withNormalizedFoundryState(state);
