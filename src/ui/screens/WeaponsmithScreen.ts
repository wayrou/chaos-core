...
function renderCostList(entry: WeaponsmithCatalogEntry): string {
  const resourceMarkup = getResourceEntries(entry.definition.cost.resources)
  ...
  return `
    <div class="weaponsmith-cost-row">
      <span class="weaponsmith-cost-chip weaponsmith-cost-chip--wad">${entry.definition.cost.wad} WAD</span>
      ${resourceMarkup}
    </div>
  `;
}
    ...
    <div class="weaponsmith-root town-screen town-screen--hub">
      ...
      ${!unlocked ? `
        <section class="weaponsmith-locked">
          ...
          <p>Recover the Weaponsmith in the Counterweight Shaft so he can reopen his temporary HAVEN bench.</p>
        </section>
      ` : `
