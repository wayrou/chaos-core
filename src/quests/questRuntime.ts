...
import { loadCampaignProgress, type CampaignProgress } from "../core/campaign";
import type { GameState, TheaterNetworkState, TheaterRoom } from "../core/types";
import { showSystemPing } from "../ui/components/systemPing";
import type { Quest, QuestObjective, QuestState } from "./types";
import { applyQuestRewardsToState } from "./questRewards";

interface QuestTheaterRuntime {
  theaterId: string;
  operationId?: string;
  floorId?: string;
  floorOrdinal: number;
...
interface QuestRuntimeSnapshot {
  completedOperations: Set<string>;
  currentFloorOrdinal: number;
  highestFloorOrdinal: number;
  theaters: QuestTheaterRuntime[];
}

const SNAPSHOT_OBJECTIVE_TYPES = new Set<QuestObjective["type"]>([
  "secure_rooms",
...
function cloneQuestState(questState: QuestState): QuestState {
  return {
    ...questState,
...
function buildRuntimeSnapshot(
  ...
  const theatersById = new Map<string, QuestTheaterRuntime>();
...
function matchesTheater(runtime: QuestTheaterRuntime, objective: QuestObjective): boolean {
...
function matchesRoom(runtime: QuestTheaterRuntime, room: TheaterRoom, objective: QuestObjective): boolean {
...
function countMatchingRooms(
  ...
  predicate: (room: TheaterRoom) => boolean,
): number {
  return snapshot.theaters.reduce((total, runtime) => (
    total + Object.values(runtime.theater.rooms).filter((room) => matchesRoom(runtime, room, objective) && predicate(room)).length
  ), 0);
}

function countMatchingTheaters(
  snapshot: QuestRuntimeSnapshot,
  objective: QuestObjective,
  predicate: (runtime: QuestTheaterRuntime) => boolean,
): number {
  return snapshot.theaters.filter((runtime) => matchesTheater(runtime, objective) && predicate(runtime)).length;
}

function getMatchingRoomFlow(
  snapshot: QuestRuntimeSnapshot,
  objective: QuestObjective,
  flowType: "supplyFlow" | "powerFlow" | "commsFlow",
): number {
  let best = 0;
...
function getObjectiveCurrentValue(objective: QuestObjective, snapshot: QuestRuntimeSnapshot): number {
  switch (objective.type) {
    ...
    case "complete_floor": {
      ...
      const floors = new Map<number, QuestTheaterRuntime[]>();
      snapshot.theaters.forEach((runtime) => {
    ...
    case "build_core":
      return countMatchingRooms(snapshot, objective, (room) => (
        Boolean(room.coreAssignment)
...
export function syncQuestProgressFromSnapshotState(
  state: GameState,
