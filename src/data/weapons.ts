...
import type {
  ...
  WeaponClutchEffect,
  WeaponHeatProfile,
  WeaponHeatZoneProfile,
  WeaponOverheatEffect,
} from "../core/weaponData";
...
function legacyHeatZones(profile: WeaponHeatProfile) {
...
const emberclawHeat = createHeatProfile(
  8,
  2,
  [
  ...
  "Locks for 1 turn and takes 2 strain to clear.",
  [{ kind: "weapon_card_lock_turns", turns: 1 }, { kind: "self_strain", amount: 2 }],
);

const steamburstHeat = createHeatProfile(
  5,
  1,
  [
  ...
  "Pushes the user back 1 tile and deals 2 self-damage.",
  [{ kind: "push_self", tiles: 1 }, { kind: "self_damage_flat", amount: 2 }],
);

const vulcanHeat = createHeatProfile(
  10,
  3,
  [
  ...
  "Cards tied to this weapon are disabled for 2 turns.",
  [{ kind: "weapon_card_lock_turns", turns: 2 }],
);

const brassbackHeat = createHeatProfile(
  6,
  1,
  [
    { min: 0, max: 2, name: "Stable", effectText: null },
  ...
  "Skips the next attack phase.",
  [{ kind: "skip_attack_turns", turns: 1 }],
);

const ironwhisperHeat = createHeatProfile(
  4,
  2,
  [
  ...
  "Destroys the currently loaded bolt.",
  [{ kind: "waste_ammo", amount: 1 }],
);

const stormlashHeat = createHeatProfile(
  9,
  1,
  [
    { min: 0, max: 4, name: "Stable", effectText: null },
  ...
  "Stuns the wielder for 1 turn.",
  [{ kind: "apply_status_self", status: "stunned", duration: 1 }],
);

const blazefangHeat = createHeatProfile(
  5,
  1,
  [
    { min: 0, max: 2, name: "Stable", effectText: null },
  ...
  "Permanently reduces max heat by 1 for the battle.",
  [{ kind: "reduce_max_heat", amount: 1 }],
);

const gearspikeHeat = createHeatProfile(
  7,
  2,
  [
    { min: 0, max: 3, name: "Stable", effectText: null },
  ...
  "Deals 3 self-damage.",
  [{ kind: "self_damage_flat", amount: 3 }],
);

const emberdrakeHeat = createHeatProfile(
  6,
  1,
  [
    { min: 0, max: 2, name: "Stable", effectText: null },
  ...
  "Cord breaks and the weapon is disabled for 2 turns.",
  [{ kind: "weapon_disable_turns", turns: 2 }],
);

const thunderjawHeat = createHeatProfile(
  12,
  2,
  [
  ...
  "Lose 1 random weapon card for the battle.",
  [{ kind: "remove_random_weapon_card", count: 1 }],
);
...
export const STARTER_WEAPONS: WeaponEquipment[] = [
  ...
  },
  {
    ...
    heatZones: legacyHeatZones(brassbackHeat).map((zone) =>
      zone.name === "Spread Boost" ? { ...zone, effect: "+1 range" } : zone,
    ),
