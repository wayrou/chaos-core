...
// MAIL MANAGEMENT
// ============================================================================
export function getMailState(state) {
    return state.quarters?.mail ?? { inbox: [] };
}
export function addMail(mailId, receivedAt = Date.now()) {
...
export function getMailById(state, mailId) {
    const mailState = getMailState(state);
    return mailState.inbox.find((mail) => mail.id === mailId) ?? null;
}
export function syncImportedMailUnlocks() {
...
// MAIL TRIGGERS
// ============================================================================
export function triggerMailOnOperationComplete(success) {
...
export function triggerMailOnBaseCampReturn() {
    if (Math.random() < 0.1) {
