...
// ============================================================================
import { addClassXP, createDefaultClassProgress, getAvailableClasses, unlockEligibleClasses, } from "./classes";
...
export function getDispatchState(state) {
    return {
        ...createInitialDispatchState(),
        ...(state.dispatch || {}),
    };
}
export function getDispatchMissionBoard() {
    return DISPATCH_MISSION_BOARD;
}
export function getBusyDispatchUnitIds(state) {
    return new Set(getDispatchState(state).activeExpeditions.flatMap((expedition) => expedition.assignedUnitIds));
}
export function getDispatchEligibleUnits(state) {
...
export function estimateDispatchSuccessChance(state, missionId, unitIds) {
...
export function launchDispatchExpedition(state, missionId, unitIds) {
...
export function advanceDispatchTime(state, ticks = 1) {
    if (ticks <= 0)
        return state;
    const dispatch = getDispatchState(state);
...
export function consumeDispatchIntelForOperation(state) {
    const dispatch = getDispatchState(state);
...
export function clearDispatchIntelBonus(state) {
    const dispatch = getDispatchState(state);
...
export function claimDispatchReport(state, reportId) {
    const dispatch = getDispatchState(state);
