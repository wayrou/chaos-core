import { NETWORK_PLAYER_SLOTS, SESSION_PLAYER_SLOTS, } from "./types";
import { createEmptyResourceWallet } from "./resources";
import { createSquadOnlineMatch, loadSquadMatchState, parseSquadMatchSnapshot, rehydrateSquadMatchState, serializeSquadMatchSnapshot, } from "./squadOnline";
...
function createResourcePool(wad = 0, resources = createEmptyResourceWallet()) {
    return {
        wad,
...
export function createHostedMultiplayerLobby(callsign, returnContext = null) {
...
export function createJoiningMultiplayerLobby(callsign, returnContext = null) {
...
export function getLobbyMembers(lobby) {
    ...
    return NETWORK_PLAYER_SLOTS.map((slot) => lobby.members[slot]).filter((member) => Boolean(member));
}
export function isLobbyFighterSlot(lobby, slot) {
...
export function shouldRenderLobbyAvatar(lobby, slot) {
...
export function findNextOpenLobbySlot(lobby) {
    return NETWORK_PLAYER_SLOTS.find((slot) => !lobby.members[slot]) ?? null;
}
export function findReconnectableLobbySlot(lobby, callsign) {
...
export function setLobbyLocalSlot(lobby, slot) {
    return {
        ...lobby,
        localSlot: slot,
        updatedAt: Date.now(),
...
export function upsertLobbyMember(lobby, slot, callsign, authorityRole, presence) {
    const now = Date.now();
...
export function removeLobbyMember(lobby, slot) {
...
export function markLobbyMemberDisconnected(lobby, slot) {
    const member = lobby.members[slot];
    if (!member) {
        return lobby;
    }
...
export function updateLobbyAvatar(lobby, slot, avatar) {
...
export function createPendingSkirmishChallenge(lobby, challengerSlot, challengeeSlot, playlist) {
...
export function updateChallengeStatus(lobby, status) {
...
export function clearPendingChallenge(lobby) {
    if (!lobby.pendingChallenge) {
        return lobby;
    }
    return {
...
export function createLobbySkirmishActivity(lobby, playlist, match) {
    const now = Date.now();
...
export function updateLobbySkirmishSnapshot(lobby, match) {
...
export function updateLobbySkirmishBattlePayload(lobby, battlePayload) {
    if (lobby.activity.kind !== "skirmish") {
        return lobby;
    }
    return {
...
export function setLobbySkirmishIntermission(lobby, match) {
    if (lobby.activity.kind !== "skirmish") {
        return lobby;
    }
    return {
...
export function chooseLobbySkirmishNextRoundDecision(lobby, decision) {
    if (lobby.activity.kind !== "skirmish") {
        return lobby;
    }
    return {
...
export function advanceLobbySkirmishRound(lobby, match) {
    if (lobby.activity.kind !== "skirmish") {
        return lobby;
    }
...
export function clearLobbyActivity(lobby) {
...
export function startCoopOperationsActivity(lobby, selectedSlots) {
    const now = Date.now();
...
export function updateCoopOperationsSelection(lobby, selectedSlots) {
...
export function setCoopOperationsSharedCampaign(lobby, sharedCampaignSlot, sharedCampaignLabel, sharedCampaignLastSavedAt = null) {
    if (lobby.activity.kind !== "coop_operations") {
        return lobby;
    }
    return {
...
export function setCoopOperationsEconomyState(lobby, options) {
    if (lobby.activity.kind !== "coop_operations") {
        return lobby;
    }
    return {
...
export function launchCoopOperationsActivity(lobby, options = {}) {
    if (lobby.activity.kind !== "coop_operations") {
        return lobby;
    }
    const now = Date.now();
...
export function syncCoopOperationsRuntime(lobby, state) {
    if (lobby.activity.kind !== "coop_operations") {
        return lobby;
    }
...
export function getLobbyLocalSkirmishMatch(lobby) {
...
export function createLobbyPlaylist(rounds) {
    return {
...
export function saveLobbyState(lobby) {
    try {
        if (!lobby) {
...
export function createResumableCoopOperationsLobby(state, returnContext = null) {
    ...
    const selectedSlots = SESSION_PLAYER_SLOTS.filter((slot) => shouldResumeCoopSessionSlot(state, slot));
...
export function withNormalizedLobbyState(state) {
    if (state.lobby) {
...
export function createSkirmishMatchFromPlaylist(challengerCallsign, challengeeCallsign, round) {
    let match = createSquadOnlineMatch(challengerCallsign, 2, round.objectiveType, round.gridWidth, round.gridHeight, round.mapId ?? null);
    match = {
        ...match,
