import { advanceTurn, confirmPlacement, createTestBattleForCurrentParty, evaluateBattleOutcome, performAutoBattleTurn, performEnemyTurn, quickPlaceUnits, } from "./battle";
...
import { buildTheaterDeploymentLaunchPreview, THEATER_SQUAD_UNIT_LIMIT, } from "./theaterDeploymentPreset";
import { createEmptyFortificationPips, getFieldAssetBuildCost, getCoreIncomeForRoom, getInstalledFortificationCount as getSchemaInstalledFortificationCount, isCoreTypeUnlocked, isFieldAssetUnlocked, isFortificationUnlocked, normalizeTheaterRoomNaturalStock, normalizeFortificationPips, roomHasTag, SCHEMA_CORE_DEFINITIONS, SCHEMA_FIELD_ASSET_DEFINITIONS, SCHEMA_FORTIFICATION_DEFINITIONS, } from "./schemaSystem";
import { CONSUMABLE_DATABASE } from "./crafting";
import { ANNEX_FRAME_DEFINITIONS, FOUNDRY_MODULE_DEFINITIONS, FOUNDRY_PARTITION_DEFINITIONS, isModuleTypeUnlocked, isPartitionTypeUnlocked, } from "./foundrySystem";
import { canTraverseTheaterEdge, createEmptyTheaterAutomationState, evaluateTheaterAutomation, findTheaterNodeRoute, getAutomatedTurretCountForRoom, getTheaterCurrentNodeId, getTheaterEdgeId, getTheaterNodeAdjacency, getTheaterNodeMoveTickCost, getTheaterRootRoomForNode, getTheaterRootRoomIdForNode, getTheaterSelectedNodeId, isTheaterNodeSecured, normalizeTheaterAnnexes, normalizeTheaterAutomationState, normalizeTheaterPartitions, resolveTheaterNode, } from "./theaterAutomation";
import { applyTacticalMapToBattleState, assignBattleUnitsToSpawnPoints, } from "./tacticalBattle";
import { cloneTacticalMapDefinition, createPointKey, getTacticalMapById, } from "./tacticalMaps";
import { addResourceWallet as addResourceWalletValues, createEmptyResourceWallet, formatResourceLabel, getResourceEntries, RESOURCE_KEYS, } from "./resources";
import { grantSessionResources, spendSessionCost, } from "./session";
import { applyShakenStatusToUnitIds, expireShakenStatusesForTheater, } from "./operationStatuses";
...
export const THEATER_CORE_BLUEPRINTS = SCHEMA_CORE_DEFINITIONS;
const FORTIFICATION_COSTS = Object.fromEntries(Object.entries(SCHEMA_FORTIFICATION_DEFINITIONS).map(([fortificationType, definition]) => [
    fortificationType,
    { ...definition.buildCost },
]));
const THEATER_STARTER_RESERVE = createEmptyResourceWallet({
    metalScrap: 10,
    wood: 8,
    chaosShards: 3,
    steamComponents: 3,
...
export function getTheaterRoomBattleMapId(room) {
    return room.battleMapId ?? getDefaultBattleMapIdForRoom(room);
}
export function getTheaterRoomTacticalMap(room) {
...
export function formatTheaterKeyLabel(keyType) {
    switch (keyType) {
        case "triangle":
            return "Triangle Key";
        case "square":
...
export function hasTheaterKey(theater, keyType) {
    if (!keyType) {
        return true;
    }
    return Boolean(theater.definition.floorKeyInventory?.[keyType]);
}
export function isTheaterRoomLocked(theater, room) {
...
function resolveAtlasSummaryForOperation(operation) {
    return ((operation.atlasTheaterId ? getAtlasTheaterSummary(operation.atlasTheaterId) : null)
...
function createRoom(definition, room) {
    const naturalStock = normalizeTheaterRoomNaturalStock(room.tags, room.naturalResourceStock, room.naturalResourceStockMax);
...
function cloneTheater(theater) {
    return {
        ...
        rooms: Object.fromEntries(Object.entries(theater.rooms).map(([roomId, room]) => [
            roomId,
            (() => {
                const naturalStock = normalizeTheaterRoomNaturalStock(room, room.naturalResourceStock, room.naturalResourceStockMax);
                return {
                    ...room,
...
export function getPreparedTheaterOperation(state) {
...
export function getPreparedTheaterOperationForSessionSlot(state, slot) {
    const operation = ensureOperationHasTheater(state.operation);
    if (!operation?.theater) {
        return operation;
    }
...
export function getPendingTheaterBattleConfirmationForSessionSlot(state, slot) {
...
export function createScopedTheaterStateForSessionSlot(state, slot) {
    if (!slot) {
        return null;
    }
...
export function mergeScopedTheaterStateForSessionSlot(baseState, scopedState, slot) {
    if (!slot) {
        return baseState;
    }
    const scopedOperation = ensureOperationHasTheater(scopedState.operation);
...
export function selectTheaterSquadForSessionSlot(state, slot, squadId) {
...
export function setTheaterCurrentRoomForSessionSlot(state, slot, roomId) {
    const scopedState = createScopedTheaterStateForSessionSlot(state, slot);
    if (!scopedState) {
        return state;
    }
...
export function moveToTheaterRoomForSessionSlot(state, slot, roomId) {
    const scopedState = createScopedTheaterStateForSessionSlot(state, slot);
    if (!scopedState) {
        return {
            state,
...
export function holdPositionInTheaterForSessionSlot(state, slot, ticks = 1) {
    const scopedState = createScopedTheaterStateForSessionSlot(state, slot);
    if (!scopedState) {
        return {
            state,
...
export function issueTheaterRoomCommandForSessionSlot(state, slot, roomId) {
    const operation = getPreparedTheaterOperationForSessionSlot(state, slot);
...
export function refuseTheaterDefenseForSessionSlot(state, slot, roomId) {
    const scopedState = createScopedTheaterStateForSessionSlot(state, slot);
    if (!scopedState) {
        return { state, success: false, message: "No active theater operation." };
    }
...
export function canManuallyControlTheaterSquad(theater, squad) {
    if (!squad) {
        return false;
    }
...
export function getTheaterPassagePowerRequirement(room, adjacentId) {
    return Math.max(0, Math.ceil((room.powerGateWatts?.[adjacentId] ?? 0) * 0.5));
}
export function isTheaterPassagePowered(theater, fromRoomId, toRoomId) {
    ...
    Object.values(next.rooms).forEach((room) => {
        ...
        getRoomOperationalCoreAssignments(room).forEach((coreAssignment) => {
            const roomIncome = (coreAssignment.type === "mine" || coreAssignment.type === "refinery")
                ...
                : createEmptyResourceWallet(Object.fromEntries(RESOURCE_KEYS.map((key) => [key, Math.max(0, (coreAssignment.incomePerTick?.[key] ?? 0) * ticks)])));
            RESOURCE_KEYS.forEach((key) => {
...
export function ensureOperationHasTheater(operation) {
    if (!operation) {
        return null;
    }
...
export function hasTheaterOperation(operation) {
    return Boolean(operation?.theater);
}
export function getMoveTickCost(theater, roomId) {
    const room = theater.rooms[roomId];
    if (!room) {
        return 0;
    }
...
export function setTheaterSelectedRoom(state, roomId) {
...
export function setTheaterCurrentRoom(state, roomId) {
    const operation = getPreparedTheaterOperation(state);
    if (!operation?.theater || !operation.theater.rooms[roomId]) {
        return state;
    }
...
export function getTheaterSelectedNode(theater) {
...
export function setTheaterSelectedNode(state, nodeId) {
    const operation = getPreparedTheaterOperation(state);
    if (!operation?.theater) {
        return state;
    }
...
export function getTheaterRoomModuleSlotUpgradeCost(room) {
...
export function buildTheaterAnnex(state, parentNodeId, frameType, edge) {
    const operation = getPreparedTheaterOperation(state);
    const theater = operation?.theater;
...
export function upgradeTheaterRoomModuleSlots(state, roomId) {
    const operation = getPreparedTheaterOperation(state);
    const theater = operation?.theater;
    if (!operation || !theater) {
        return { state, success: false, message: "No active theater operation." };
...
export function installTheaterModule(state, nodeId, moduleType) {
    const operation = getPreparedTheaterOperation(state);
    const theater = operation?.theater;
...
export function removeTheaterModule(state, moduleId) {
    const operation = getPreparedTheaterOperation(state);
    const theater = operation?.theater;
    if (!operation || !theater) {
        return { state, success: false, message: "No active theater operation." };
    ...
    return {
        state: buildConfiguredTheaterState(state, operation, nextTheater, `AUTOMATION :: ${FOUNDRY_MODULE_DEFINITIONS[moduleInstance.moduleType]?.displayName ?? moduleInstance.moduleType} removed from ${node?.label ?? moduleInstance.installedNodeId}.`),
        success: true,
        message: "Module removed.",
    };
}
export function configureTheaterModule(state, moduleId, configuration) {
    const operation = getPreparedTheaterOperation(state);
    const theater = operation?.theater;
    if (!operation || !theater) {
        return { state, success: false, message: "No active theater operation." };
    ...
    nextTheater.selectedNodeId = moduleInstance.installedNodeId;
    return {
        state: buildConfiguredTheaterState(state, operation, nextTheater, `AUTOMATION :: ${FOUNDRY_MODULE_DEFINITIONS[moduleInstance.moduleType]?.displayName ?? moduleInstance.moduleType} configuration updated.`),
        success: true,
        message: "Module configuration updated.",
    };
}
export function resetTheaterModuleState(state, moduleId) {
    const operation = getPreparedTheaterOperation(state);
    const theater = operation?.theater;
    if (!operation || !theater) {
        return { state, success: false, message: "No active theater operation." };
...
export function installTheaterPartition(state, nodeAId, nodeBId, partitionType) {
    const operation = getPreparedTheaterOperation(state);
    const theater = operation?.theater;
...
export function toggleTheaterPartitionState(state, edgeId, nextStateOverride) {
    const operation = getPreparedTheaterOperation(state);
    const theater = operation?.theater;
    if (!operation || !theater) {
        return { state, success: false, message: "No active theater operation." };
    ...
    return {
        state: buildConfiguredTheaterState(state, operation, nextTheater, `PARTITION :: ${FOUNDRY_PARTITION_DEFINITIONS[partition.partitionType]?.displayName ?? "Partition"} ${partition.state.toUpperCase()} on ${edgeId}.`),
        success: true,
        message: `${partition.state === "open" ? "Opened" : "Closed"} ${FOUNDRY_PARTITION_DEFINITIONS[partition.partitionType]?.displayName ?? "partition"}.`,
    };
}
export function setTheaterRoomSignalPosture(state, roomId, posture) {
    const operation = getPreparedTheaterOperation(state);
    const theater = operation?.theater;
    if (!operation || !theater) {
        return { state, success: false, message: "No active theater operation." };
...
export function setTheaterRoomContainmentMode(state, roomId, mode) {
    const operation = getPreparedTheaterOperation(state);
    const theater = operation?.theater;
    if (!operation || !theater) {
        return { state, success: false, message: "No active theater operation." };
...
export function triggerTheaterEmergencyDump(state, roomId) {
    const operation = getPreparedTheaterOperation(state);
    const theater = operation?.theater;
    if (!operation || !theater) {
        return { state, success: false, message: "No active theater operation." };
...
export function moveToTheaterNode(state, nodeId) {
    const operation = getPreparedTheaterOperation(state);
    const theater = operation?.theater;
    if (!operation || !theater) {
        return {
...
function advanceMountedTheaterRuntimeTicks(state, theater, ticks) {
    ...
    for (let tickIndex = 0; tickIndex < ticks; tickIndex += 1) {
        ...
        const afterThreats = applySandboxSlice(evaluateTheaterAutomation(resolveEnemyRoomThreats(nextState, nextTheater)), activeTheater, true);
        nextState = {
            ...nextState,
            operation: resolveOperationFields(operation, afterThreats),
        };
        if (operation.id) {
            nextState = expireShakenStatusesForTheater(nextState, operation.id, nextTheater.definition.id, nextTheater.tickCount);
        }
...
export function moveToTheaterRoom(state, roomId) {
    const operation = getPreparedTheaterOperation(state);
    const theater = operation?.theater;
    if (!operation || !theater) {
        return {
...
export function holdPositionInTheater(state, ticks = 1) {
    const operation = getPreparedTheaterOperation(state);
    const theater = operation?.theater;
    if (!operation || !theater) {
        return {
...
export function getSelectedTheaterSquad(theater) {
    return getSelectedSquad(theater);
}
export function getTheaterObjectiveDefinition(theater) {
...
export function selectTheaterSquad(state, squadId) {
    const operation = getPreparedTheaterOperation(state);
    const theater = operation?.theater;
    if (!operation || !theater) {
        return { state, success: false, message: "No active theater operation." };
...
export function setTheaterSquadAutomationMode(state, squadId, automationMode) {
    const operation = getPreparedTheaterOperation(state);
    const theater = operation?.theater;
    if (!operation || !theater) {
        return { state, success: false, message: "No active theater operation." };
...
export function renameTheaterSquad(state, squadId, displayName) {
    const operation = getPreparedTheaterOperation(state);
    const theater = operation?.theater;
    if (!operation || !theater) {
        return { state, success: false, message: "No active theater operation." };
...
export function useTheaterConsumable(state, targetUnitId, consumableId) {
    const operation = getPreparedTheaterOperation(state);
    const theater = operation?.theater;
    if (!operation || !theater) {
        return { state, success: false, message: "No active theater operation." };
...
export function setTheaterSquadIcon(state, squadId, icon) {
    const operation = getPreparedTheaterOperation(state);
    const theater = operation?.theater;
    if (!operation || !theater) {
        return { state, success: false, message: "No active theater operation." };
...
export function setTheaterSquadColor(state, squadId, colorKey) {
    const operation = getPreparedTheaterOperation(state);
    const theater = operation?.theater;
    if (!operation || !theater) {
        return { state, success: false, message: "No active theater operation." };
    ...
    nextSquad.colorKey = normalizeSquadColorKey(colorKey, THEATER_SQUAD_COLOR_CHOICES.indexOf(nextSquad.colorKey));
    return {
        state: {
            ...state,
            phase: "operation",
...
export function splitUnitToNewSquad(state, squadId, unitId) {
    const operation = getPreparedTheaterOperation(state);
    const theater = operation?.theater;
    if (!operation || !theater) {
        return { state, success: false, message: "No active theater operation." };
...
export function transferUnitBetweenSquads(state, fromSquadId, toSquadId, unitId) {
    const operation = getPreparedTheaterOperation(state);
    const theater = operation?.theater;
    if (!operation || !theater) {
        return { state, success: false, message: "No active theater operation." };
...
export function mergeTheaterSquads(state, fromSquadId, intoSquadId) {
    const operation = getPreparedTheaterOperation(state);
    const theater = operation?.theater;
    if (!operation || !theater) {
        return { state, success: false, message: "No active theater operation." };
...
export function refuseTheaterDefense(state, roomId) {
    const operation = getPreparedTheaterOperation(state);
    const theater = operation?.theater;
    if (!operation || !theater) {
        return { state, success: false, message: "No active theater operation." };
...
export function getTheaterCoreRepairCost(room) {
    return deriveTheaterCoreRepairCost(room);
}
export function repairTheaterCore(state, roomId) {
    const operation = getPreparedTheaterOperation(state);
    const theater = operation?.theater;
    if (!operation || !theater) {
        return { state, success: false, message: "No active theater operation." };
...
export function destroyTheaterCore(state, roomId, slotIndex) {
    const operation = getPreparedTheaterOperation(state);
    const theater = operation?.theater;
    if (!operation || !theater) {
        return { state, success: false, message: "No active theater operation." };
...
export function destroyTheaterFortification(state, roomId, fortificationType) {
    const operation = getPreparedTheaterOperation(state);
    const theater = operation?.theater;
...
export function destroyTheaterAnnex(state, annexId) {
    const operation = getPreparedTheaterOperation(state);
    const theater = operation?.theater;
    if (!operation || !theater) {
        return { state, success: false, message: "No active theater annex target." };
...
export function secureTheaterRoomInState(state, roomId, squadId) {
    const operation = getPreparedTheaterOperation(state);
    if (!operation?.theater) {
        return state;
    }
...
export function clearTheaterEnemyThreatsInState(state) {
    const operation = getPreparedTheaterOperation(state);
    const theater = operation?.theater;
    if (!operation || !theater) {
        return state;
...
export function buildCoreInTheaterRoom(state, roomId, coreType) {
    const operation = getPreparedTheaterOperation(state);
...
export function fortifyTheaterRoom(state, roomId, fortificationType) {
    const operation = getPreparedTheaterOperation(state);
    if (!operation?.theater) {
        return {
            state,
...
export function getFieldAssetPlacementError(room, fieldAssetType, x, y) {
...
export function fabricateFieldAssetInTheaterRoom(state, roomId, fieldAssetType, x, y) {
    const operation = getPreparedTheaterOperation(state);
    if (!operation?.theater) {
        return {
            state,
...
export function removeFieldAssetFromTheaterRoom(state, roomId, assetId) {
    const operation = getPreparedTheaterOperation(state);
    if (!operation?.theater) {
        return {
            state,
...
export function applyTheaterOperationFailure(state, battle) {
...
export function clearCurrentTheaterOperationInjuries(state) {
    const operation = getPreparedTheaterOperation(state);
    const theater = operation?.theater;
    if (!operation || !theater) {
        return state;
...
export function createTheaterBattleState(state, roomId, squadId) {
    const operation = getPreparedTheaterOperation(state);
    const theater = operation?.theater;
    if (!operation || !theater) {
        return null;
...
export function applyTheaterBattleOutcome(state, battle) {
...
export function resolveTheaterAutoBattle(state, squadId, roomId, mode) {
    const operation = getPreparedTheaterOperation(state);
    const theater = operation?.theater;
    if (!operation || !theater) {
        return state;
...
export function getTheaterSummary(state) {
    const operation = getPreparedTheaterOperation(state);
    const theater = operation?.theater;
...
export function getTheaterStarterResources() {
    return { ...THEATER_STARTER_RESERVE };
}
export function getFortificationCost(type) {
...
export function getTheaterUpkeepPerTick(theater) {
...
export function getTheaterRoomCoreAssignments(room) {
    return getRoomCoreAssignments(room);
}
export function getTheaterRoomOpenCoreSlots(room) {
    return Math.max(0, Math.max(1, room.coreSlotCapacity ?? 1) - getRoomCoreAssignments(room).length);
}
export function isTheaterCoreOperational(room) {
    return roomHasOperationalCore(room);
}
export function getTheaterCoreOperationalRequirements(room) {
    return getRoomCoreOperationalRequirements(room);
}
export function getTheaterCoreOfflineReason(room) {
    return getCoreOfflineReason(room);
}
export function hasCompletedTheaterObjective(theater) {
    return theater.objectiveComplete && theater.completion !== null;
}
export function recomputeTheaterNetwork(theater) {
