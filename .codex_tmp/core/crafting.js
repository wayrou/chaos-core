...
import { RESOURCE_KEYS, formatResourceLabel, hasEnoughResources, } from "./resources";
// ----------------------------------------------------------------------------
// RECIPE DATABASE
// ----------------------------------------------------------------------------
 ...
 */
export function getKnownRecipes(knownRecipeIds) {
    return knownRecipeIds
        .map(id => RECIPE_DATABASE[id])
        .filter((r) => r !== undefined && !isTechnicaContentDisabled("item", r.resultItemId));
}
/**
 * Get all starter recipes (known by default)
 */
export function getStarterRecipes() {
    return Object.values(RECIPE_DATABASE).filter(r => r.starterRecipe && !isTechnicaContentDisabled("item", r.resultItemId));
}
/**
 * Get starter recipe IDs
 */
export function getStarterRecipeIds() {
 ...
 */
export function hasRequiredItem(recipe, inventoryItemIds) {
 ...
 */
export function getRecipeCostString(recipe) {
 ...
 */
export function getActiveRecipes(recipes) {
    return recipes.filter(r => !r.deprecated);
}
/**
 * Get deprecated recipes (for migration/history purposes)
 */
export function getDeprecatedRecipes(recipes) {
    return recipes.filter(r => r.deprecated);
}
/**
 * Learn a new recipe
 */
export function learnRecipe(knownRecipeIds, recipeId) {
    if (knownRecipeIds.includes(recipeId))
        return knownRecipeIds;
    if (!RECIPE_DATABASE[recipeId])
        return knownRecipeIds;
    return [...knownRecipeIds, recipeId];
}
export function craftItem(recipe, resources, inventoryItemIds) {
