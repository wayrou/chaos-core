...
export function normalizeFieldDecorRotationQuarterTurns(rotationQuarterTurns) {
    const normalized = Math.round(Number(rotationQuarterTurns ?? 0));
    const wrapped = ((normalized % 4) + 4) % 4;
    return wrapped;
}
export function getFieldDecorFootprintSize(decor, rotationQuarterTurns) {
...
export function getPlacedFieldDecor(state, mapId) {
...
export function getUnplacedDecor(state) {
    const decorState = getDecorState(state);
...
export function placeDecor(decorId, anchorId) {
...
export function removeDecor(anchorId) {
    updateGameState((current) => {
        const currentDecorState = getDecorState(current);
        const nextPlaced = { ...currentDecorState.placedDecorByAnchor };
...
export function placeFieldDecor(decorId, mapId, x, y, rotationQuarterTurns = 0) {
    const state = getGameState();
    const decor = getDecorItemById(decorId);
...
export function moveFieldDecor(placementId, x, y) {
    const state = getGameState();
    const decorState = getDecorState(state);
...
export function setFieldDecorRotation(placementId, rotationQuarterTurns) {
    const state = getGameState();
    const decorState = getDecorState(state);
    if (!(decorState.fieldPlacements ?? []).some((placement) => placement.placementId === placementId)) {
        return false;
...
export function removeFieldDecor(placementId) {
    const state = getGameState();
    const decorState = getDecorState(state);
    if (!(decorState.fieldPlacements ?? []).some((placement) => placement.placementId === placementId)) {
        return false;
...
export function grantDecorItem(decorId) {
    const state = getGameState();
    const decorState = getDecorState(state);
    if (decorState.owned.includes(decorId)) {
        return false;
...
export function craftDecorItem(decorId) {
    const state = getGameState();
    const decor = getDecorItemById(decorId);
    if (!decor || decor.sourceRules?.craftEligible === false) {
        return false;
...
export function seedInitialDecor() {
    const state = getGameState();
    const decorState = getDecorState(state);
