...
import { createEmptyFortificationPips, normalizeTheaterRoomNaturalStock, normalizeFortificationPips, } from "./schemaSystem";
import { createEmptyTheaterAutomationState } from "./theaterAutomation";
function getDefaultModuleSlotCapacity(room) {
...
function createPresentation(operation, floorIndex, atlasSummary, rng) {
    ...
    const layoutStyles = preferredDirection
    ...
    const mapAnchor = edgeBucket === "east"
...
function pickMegaRoomSeed(roomSeeds, rng) {
    ...
    const candidates = roomSeeds
        ...
        .sort((left, right) => (right.depthFromUplink - left.depthFromUplink
...
function pickStagingRoomSeed(roomSeeds, excludedRoomId, rng) {
    const candidates = roomSeeds
        .filter((seed) => (seed.id !== excludedRoomId
        && !seed.isUplinkRoom
        && !seed.secured
        && !seed.tags.includes("objective")
...
function jitterTemplatePosition(basePosition, role, style, rng, index) {
    ...
    const forwardVariance = role === "objective"
        ? 0.16
        : role === "elite"
            ? 0.28
            : 0.44;
    const lateralVariance = style === "central_bloom"
        ? 0.95
        : style === "offset_arc"
            ? 0.82
            : 0.72;
    const laneBias = Math.abs(basePosition.y) < 0.35 && role !== "objective"
...
function createExpandedLayoutTemplate(style, rng) {
    const baseTemplate = (LAYOUT_TEMPLATES[style] ?? LAYOUT_TEMPLATES.vector_lance).map((node, index) => ({
        ...node,
        localPosition: jitterTemplatePosition(stretchTemplatePosition(node.localPosition, node.role, style, rng), node.role, style, rng, index),
    ...
    for (let index = 0; index < extraRoomCount; index++) {
        const eligibleParents = baseTemplate.filter((node) => (node.role !== "objective" &&
        ...
        if (rng.nextFloat() < 0.42) {
            const secondaryCandidates = baseTemplate.filter((candidate) => (candidate.key !== spurKey &&
...
function buildRoomSeeds(_operation, floorIndex, profile, definition, style, rng) {
    ...
    const rooms = Object.fromEntries(roomSeeds.map((room) => [room.id, createRoom(definition, room)]));
...
function hasAlternateEdgeRoute(rooms, fromRoomId, toRoomId) {
    ...
    while (queue.length > 0) {
        ...
        for (const adjacentId of room.adjacency) {
            const isIgnoredDirectEdge = (currentRoomId === fromRoomId && adjacentId === toRoomId)
...
export function createGeneratedTheaterFloor(operation, floorIndex) {
    ...
    const objective = operation.objective ?? (isFinalFloor
    ...
    const endState = operation.endState ?? (isFinalFloor
