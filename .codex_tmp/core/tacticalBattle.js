...
import { createTrainingEncounter } from "./trainingEncounter";
import { cloneTacticalMapDefinition, createPointKey, } from "./tacticalMaps";
...
export function createBattleTilesFromTacticalMap(map) {
    return map.tiles.map((tile) => createTileFromMapPoint(map, tile));
}
export function applyTacticalMapToBattleState(battle, sourceMap) {
...
export function createSquadObjectiveStateFromTacticalMap(map, objectiveType) {
...
export function assignBattleUnitsToSpawnPoints(battle, side, points) {
    if (points.length <= 0) {
        return battle;
    }
...
export function createBuilderQuickTestBattle(state, sourceMap, objectiveType = null) {
    const map = cloneTacticalMapDefinition(sourceMap);
...
export function getTacticalMapSpawnCapacity(map) {
