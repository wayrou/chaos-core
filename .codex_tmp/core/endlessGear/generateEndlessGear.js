...
import { createSeededRNG, generateSeed, weightedRandom, randomInt } from "../rng";
import { getChassisById, ALL_CHASSIS } from "../../data/gearChassis";
import { getDoctrineById, ALL_DOCTRINES } from "../../data/gearDoctrines";
 ...
 */
export function generateEndlessGearFromRecipe(recipe, ctx, options) {
    ...
    const stability = Math.max(0, Math.min(100, randomInt(rng, bias.stabilityRange.min, bias.stabilityRange.max)));
 ...
 */
export function generateEndlessLoot(ctx, params = {}) {
    ...
    const materials = Array.from({ length: materialCount }, () => allMaterials[randomInt(rng, 0, allMaterials.length - 1)]);
    // Create recipe
    const recipe = {
 ...
 */
export function explainEndlessGear(gear) {
