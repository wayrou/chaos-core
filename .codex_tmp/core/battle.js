...
import { buildDeckFromLoadout, calculateEquipmentStats, getAllStarterEquipment, } from "./equipment";
import { advanceWeaponTurn, addHeat, createWeaponRuntimeState, getEffectiveMaxHeat, rollWeaponHit, rollWeaponNodeHit, damageNode, triggerWeaponOverheat, WEAPON_NODE_NAMES, } from "./weaponSystem";
import { getResolvedBattleCard } from "./cardCatalog";
// STEP 6 & 7: Import gear workbench functions
import { generateBattleRewardCards, getLibraryCard, } from "./gearWorkbench";
import { createBattleGearRewardSpec } from "./gearRewards";
// Mount system imports
import { getMountById, findOwnedMount, validateMountCards, } from "./mounts";
...
import { getEchoAttackBonus, getEchoAccuracyBonus, getEchoIncomingAccuracyPenalty, getEchoFirstCardStrainDiscount, getEchoOnHitVulnerable, getEchoDefenseBonus, getEchoMovementAdjustment, getEchoTurnStartCleanse, getEchoTurnStartDrawBonus, getEchoTurnStartGuarded, getEchoTurnStartHealing, getEchoTurnStartStrainRelief, incrementEchoFieldTriggerCount, isEchoBattle, } from "./echoFieldEffects";
...
export function getLoadPenalties(state) {
...
export function addStatus(state, unitId, type, duration = 1, sourceId) {
...
export function removeStatus(state, unitId, type) {
    const unit = state.units[unitId];
...
export function removeAllStatusesFromSource(state, sourceId) {
    let nextState = { ...state };
...
// GRID CREATION
// ----------------------------------------------------------------------------
export function createGrid(width, height, elevationMap) {
    const tiles = [];
 ...
 */
export function createBattleUnitState(base, opts, equipmentById) {
    // Get equipment data
    ...
    if (!usesLoadoutRules) {
        ...
        deckCards = buildDeckFromLoadout(unitClass, loadout, equipment, opts.gearSlots);
    }
...
export function getStrainThreshold() {
    return BASE_STRAIN_THRESHOLD;
}
export function isOverStrainThreshold(unit) {
    return unit.strain >= getStrainThreshold();
}
export function applyStrain(state, unit, amount) {
...
export function getSquadObjectiveControlSide(state) {
 ...
 */
export function getBattleUnitEquippedWeaponId(unit) {
    if (!unit) {
        return null;
    }
 ...
 */
export function getEquippedWeapon(unit, equipmentById) {
 ...
 */
export function updateUnitWeaponState(state, unitId, newWeaponState) {
    const unit = state.units[unitId];
    if (!unit)
        return state;
    return {
...
export function applyWeaponOverheatEffects(state, unitId, effects, sourceLabel) {
    let next = state;
...
export function applyWeaponHitToUnit(state, defenderId, isCrit = false) {
...
export function applyTheaterCombatInstability(state, unitId, trigger = "attack") {
 ...
 */
export function unitHasMountPassive(unit, trait) {
 ...
 */
export function getMountMovementBonus(unit) {
    if (!unit.mountId)
        return 0;
 ...
 */
export function getUnitMovementRange(unit, battle) {
 ...
 */
export function getChargeDamageBonus(unit, tilesMoved) {
 ...
 */
export function getArmoredDamageReduction(unit) {
 ...
 */
export function getIntimidateAccuracyPenalty(attacker, defender) {
...
// ----------------------------------------------------------------------------
export function arePositionsAdjacent(a, b) {
    const dx = Math.abs(a.x - b.x);
    const dy = Math.abs(a.y - b.y);
    return dx + dy === 1;
}
export function isInsideBounds(state, pos) {
    return (pos.x >= 0 &&
        pos.y >= 0 &&
...
export function getTraversalDestinations(state, from) {
...
export function hasTraversalBetween(state, from, to) {
    return getTraversalDestinations(state, from).some((point) => point.x === to.x && point.y === to.y);
}
export function canTraverseElevationStep(state, from, to) {
...
export function isWalkableTile(state, pos) {
...
export function canUnitMoveTo(state, unit, dest) {
    if (!unit.pos)
        return false;
...
export function getReachableMovementTiles(state, unit, origin = unit.pos ?? { x: 0, y: 0 }) {
    if (!unit.pos)
        return new Set();
    if (hasStatus(unit, "immobilized"))
        return new Set();
 ...
 */
export function getMovePath(state, start, dest, maxCost) {
...
export function getUnitInteractionObject(state, unit) {
    if (!unit?.pos) {
        return null;
    }
...
export function canUnitInteract(state, unit) {
    return Boolean(getUnitInteractionObject(state, unit));
}
export function interactWithMapObject(state, unitId) {
    const unit = state.units[unitId];
    if (!unit) {
        return state;
    }
...
export function moveUnit(state, unitId, dest) {
    const unit = state.units[unitId];
    if (!unit)
        return state;
    const updatedUnit = {
...
export function computeHitChance(attacker, defender, isRanged = false, battle) {
...
export function attackUnit(state, attackerId, defenderId) {
    ...
    if (roll > hitChance) {
        ...
        const echoedMissState = (echoAccuracyBonus.triggeredPlacements.length > 0 || echoIncomingPenalty.triggeredPlacements.length > 0)
            ? incrementEchoFieldTriggerCount(unstableMissState, [...echoAccuracyBonus.triggeredPlacements, ...echoIncomingPenalty.triggeredPlacements])
            : unstableMissState;
    ...
    if (echoAttackBonus.triggeredPlacements.length > 0
        ...
        next = incrementEchoFieldTriggerCount(next, [
    ...
    if (finalDamage > 0 && echoHex.duration > 0 && !isKill) {
        ...
        next = incrementEchoFieldTriggerCount(next, echoHex.triggeredPlacements, `SLK//ECHO  :: ${defender.name} is exposed by Hex Zone.`);
    }
...
// ENEMY AI
// ----------------------------------------------------------------------------
export function performEnemyTurn(state) {
 ...
 */
export function performAutoBattleTurn(state, unitId, policy = "daring") {
    const unit = state.units[unitId];
...
// ----------------------------------------------------------------------------
export function evaluateBattleOutcome(state) {
 ...
 */
export function drawCardsForTurn(state, unit, handSize = 5) {
    ...
    if (drawBonus.amount > 0) {
        nextState = incrementEchoFieldTriggerCount(nextState, drawBonus.triggeredPlacements, `SLK//ECHO  :: ${unit.name} draws +${drawBonus.amount} from Relay Zone.`);
    }
    return nextState;
}
// ----------------------------------------------------------------------------
 ...
 */
export function calculateMaxUnitsPerSide(gridWidth, gridHeight) {
    const gridArea = gridWidth * gridHeight;
    const rawMax = Math.floor(gridArea * 0.25);
    return Math.max(3, Math.min(rawMax, 10));
}
export function createTestBattleForCurrentParty(state, gridOverride) {
    ...
    if (state.inventory) {
        const loadPenalties = computeLoadPenaltyFlags(state.inventory);
 ...
 */
export function playCard(state, unitId, cardIndex, targetId) {
    ...
    if (overdriveDiscount.amount > 0) {
        newState = incrementEchoFieldTriggerCount(newState, overdriveDiscount.triggeredPlacements, `SLK//ECHO  :: ${unit.name} overdrives the first card for -${overdriveDiscount.amount} Strain.`);
    }
 ...
 */
export function drawCards(state, unit, _count = 5) {
    return drawCardsForTurn(state, unit);
}
export function getPlacementTilesForUnit(state, unit) {
...
export function getEffectivePlacedUnitIds(state) {
 ...
 */
export function quickPlaceUnits(state, controller) {
    if (state.phase !== "placement")
        return state;
    const placementState = state.placementState;
    if (!placementState)
 ...
 */
export function removePlacedUnit(state, unitId) {
    if (state.phase !== "placement")
        return state;
    const unit = state.units[unitId];
    if (!unit)
 ...
 */
export function setPlacementSelectedUnit(state, unitId) {
    if (state.phase !== "placement")
        return state;
    const placementState = state.placementState;
    if (!placementState)
 ...
 */
export function confirmPlacement(state) {
    if (state.phase !== "placement")
        return state;
    const placementState = state.placementState;
    if (!placementState)
