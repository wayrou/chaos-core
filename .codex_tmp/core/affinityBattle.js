...
// ============================================================================
import { recordMeleeAttack, recordRangedSkill, recordMagicSpell, recordSupportAction, recordMobilityAction, recordSurvival, } from "./affinity";
import { updateGameState } from "../state/gameStore";
/**
 * Track affinity for a melee attack
 */
export function trackMeleeAttackInBattle(attackerId, battle) {
 ...
 */
export function trackRangedSkillInBattle(unitId, battle) {
 ...
 */
export function trackMagicSpellInBattle(unitId, battle) {
    const unit = battle.units[unitId];
    if (!unit || unit.isEnemy)
        return;
    updateGameState((state) => {
 ...
 */
export function trackSupportActionInBattle(unitId, battle) {
    const unit = battle.units[unitId];
    if (!unit || unit.isEnemy)
        return;
    updateGameState((state) => {
 ...
 */
export function trackMobilityActionInBattle(unitId, battle) {
    const unit = battle.units[unitId];
    if (!unit || unit.isEnemy)
        return;
    updateGameState((state) => {
 ...
 */
export function trackBattleSurvival(battle, victory) {
    if (!victory)
 ...
 */
export function detectCardAffinityType(cardName, cardDescription) {
