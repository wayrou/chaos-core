...
export const WEAPONSMITH_UPGRADES = {
    quickdraw_limbs: {
        id: "quickdraw_limbs",
...
export function createDefaultWeaponsmithState() {
    return {
        installedUpgradeIds: [],
    };
}
export function withNormalizedWeaponsmithState(state) {
...
export function isWeaponsmithUnlocked(state) {
    return Boolean(state.outerDecks?.seenNpcEncounterIds?.includes(COUNTERWEIGHT_WEAPONSMITH_ENCOUNTER_ID));
}
export function getWeaponsmithInstalledUpgradeIds(state) {
    return [...(state.weaponsmith?.installedUpgradeIds ?? [])];
}
export function getBowbladeFieldProfile(state) {
...
export function getBowbladeWorkshopReadout(state) {
...
export function getWeaponsmithUpgradeDefinitions() {
    return WEAPONSMITH_UPGRADE_ORDER.map((upgradeId) => WEAPONSMITH_UPGRADES[upgradeId]);
}
export function isWeaponsmithUpgradeUnlocked(state, upgradeId) {
    if (!isWeaponsmithUnlocked(state)) {
        return false;
    }
    switch (upgradeId) {
...
export function getWeaponsmithCatalog(state) {
...
export function installWeaponsmithUpgrade(state, upgradeId) {
