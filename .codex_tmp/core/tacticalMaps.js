...
export function createPointKey(point) {
...
export function cloneTacticalMapDefinition(map) {
    return {
        ...map,
...
export function createTacticalMapId(prefix = "custom") {
...
export function normalizeTacticalMapDefinition(input, authorFallback = DEFAULT_AUTHOR) {
...
export function getBuiltInTacticalMaps() {
    return BUILT_IN_MAPS.map(cloneTacticalMapDefinition);
}
export function getTacticalMapTemplates() {
    return TEMPLATE_MAPS.map(cloneTacticalMapDefinition);
}
export function instantiateTemplateMap(templateId, author = DEFAULT_AUTHOR, name) {
...
export function generateProceduralTacticalMap(theme, author = DEFAULT_AUTHOR) {
...
export function loadCustomTacticalMaps() {
...
export function saveCustomTacticalMaps(maps) {
    if (!canUseLocalStorage()) {
        return;
    }
    try {
...
export function serializeTacticalMapForExport(map, authorFallback = DEFAULT_AUTHOR) {
    return JSON.stringify(normalizeTacticalMapDefinition({ ...map, isBuiltIn: false, isTemplate: false }, authorFallback), null, 2);
}
export function serializeTacticalMapToShareCode(map, authorFallback = DEFAULT_AUTHOR) {
    const payload = {
        version: 1,
        kind: "share_code",
...
export function parseTacticalMapShareCode(raw, authorFallback = DEFAULT_AUTHOR) {
...
export function serializeCustomTacticalMapLibraryForExport() {
    const payload = {
        version: TACTICAL_MAP_LIBRARY_VERSION,
...
export function parseImportedTacticalMapPayload(raw, authorFallback = DEFAULT_AUTHOR) {
    const trimmed = raw.trim();
    if (trimmed.startsWith(TACTICAL_MAP_SHARE_CODE_PREFIX)) {
        return {
            kind: "share_code",
...
export function reviewTacticalMapImports(maps, authorFallback = DEFAULT_AUTHOR) {
...
export function commitTacticalMapImportReview(reviewItems, decisions = {}, authorFallback = DEFAULT_AUTHOR) {
    const catalog = getTacticalMapCatalog();
...
export function importCustomTacticalMaps(maps, authorFallback = DEFAULT_AUTHOR) {
...
export function upsertCustomTacticalMap(map, authorFallback = DEFAULT_AUTHOR) {
...
export function deleteCustomTacticalMap(mapId) {
    saveCustomTacticalMaps(loadCustomTacticalMaps().filter((map) => map.id !== mapId));
}
export function duplicateCustomTacticalMap(mapId, authorFallback = DEFAULT_AUTHOR) {
...
export function getTacticalMapCatalog() {
    return {
...
export function getAllTacticalMaps() {
    const catalog = getTacticalMapCatalog();
    return [...catalog.builtInMaps, ...catalog.customMaps];
}
export function getTacticalMapById(mapId) {
    if (!mapId) {
        return null;
    }
    return getAllTacticalMaps().find((map) => map.id === mapId) ?? null;
}
export function mapSupportsObjectiveType(map, objectiveType) {
    return Boolean(map?.supportedModes.includes(objectiveType));
}
export function validateTacticalMapDefinition(mapInput) {
