import { completeOperation, loadCampaignProgress, saveCampaignProgress, } from "./campaign";
import { buildCampaignRegionZoneName, getActiveRegionPresentation, } from "./campaignRegions";
import { ensureOperationHasTheater, getTheaterCoreOfflineReason, getTheaterUpkeepPerTick, isTheaterCoreOperational, } from "./theaterSystem";
import { addResourceWallet as addResourceWalletValues, createEmptyResourceWallet, RESOURCE_KEYS, } from "./resources";
import { getLocalSessionPlayerSlot, getSessionResourcePool, grantSessionResources, spendSessionCost, } from "./session";
...
export function loadOpsTerminalAtlasProgress() {
...
export function getCurrentOpsTerminalAtlasFloor(progress = loadOpsTerminalAtlasProgress()) {
...
export function getOpsTerminalAtlasWarmEconomySummaries(floorOrdinal, progress = loadOpsTerminalAtlasProgress()) {
    const ensured = ensureCurrentFloorInitialized(progress);
    const atlas = decorateAtlasState(ensured.opsTerminalAtlas);
...
export function getOpsTerminalAtlasOtherWarmEconomySummaries(activeTheaterId, progress = loadOpsTerminalAtlasProgress()) {
    const ensured = ensureCurrentFloorInitialized(progress);
    const atlas = decorateAtlasState(ensured.opsTerminalAtlas);
...
export function getHighestGeneratedOpsTerminalAtlasFloorOrdinal(progress = loadOpsTerminalAtlasProgress()) {
    const ensured = ensureCurrentFloorInitialized(progress);
...
export function setCurrentOpsTerminalAtlasFloorOrdinal(floorOrdinal, progress = loadOpsTerminalAtlasProgress(), allowObjectiveBypass = false) {
    const ensured = ensureCurrentFloorInitialized(progress);
...
export function regenerateOpsTerminalAtlasFloor(floorOrdinal, progress = loadOpsTerminalAtlasProgress()) {
    const ensured = ensureCurrentFloorInitialized(progress);
    const atlas = ensured.opsTerminalAtlas ? cloneAtlasState(ensured.opsTerminalAtlas) : null;
    if (!atlas) {
        throw new Error("Ops terminal atlas state is not initialized.");
...
export function restartOpsTerminalAtlas(progress = loadOpsTerminalAtlasProgress()) {
    const ensured = ensureCurrentFloorInitialized(progress);
...
export function applyWarmTheaterEconomyToState(state, activeTheaterId, tickCost) {
    if (tickCost <= 0) {
        return state;
    }
...
export function holdPositionInOpsTerminalAtlas(state, floorOrdinal, ticks = 1) {
...
export function getOpsTerminalAtlasSectorState(theaterId, progress = loadOpsTerminalAtlasProgress()) {
...
export function createOpsTerminalSectorOperation(theaterId) {
    const progress = loadOpsTerminalAtlasProgress();
...
export function isOpsTerminalAtlasOperation(operation) {
...
export function syncOpsTerminalOperationState(operation) {
...
export function getSectorSecuredRoomCount(sector) {
    return Object.values(sector.theater.rooms).filter((room) => room.secured).length;
}
export function getSectorKnownRoomCount(sector) {
    return Object.values(sector.theater.rooms).length;
}
export function getSectorStateLabel(sector) {
    return sector.theater.definition.currentState;
}
export function getSectorCoreCount(sector) {
    return countActiveCores(sector.theater);
}
export function getOpsTerminalAtlasCoreSummaries(floorOrdinal, progress = loadOpsTerminalAtlasProgress()) {
    const ensured = ensureCurrentFloorInitialized(progress);
    const atlas = decorateAtlasState(ensured.opsTerminalAtlas);
    const resolvedFloorOrdinal = floorOrdinal ?? atlas.currentFloorOrdinal;
    const floor = atlas.floorsById[getFloorId(resolvedFloorOrdinal)];
