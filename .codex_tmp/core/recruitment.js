...
// ============================================================================
import { GUILD_ROSTER_LIMITS, } from "./types";
 ...
 */
export function generateCandidates(hub, existingRosterSize) {
    const poolSize = Math.min(hub.candidatePoolSize, GUILD_ROSTER_LIMITS.MAX_TOTAL_MEMBERS - existingRosterSize);
    if (poolSize <= 0) {
        return [];
    }
    const candidates = [];
 ...
 */
export function hireCandidate(candidateId, candidates, state) {
 ...
 */
export function isRosterFull(state) {
