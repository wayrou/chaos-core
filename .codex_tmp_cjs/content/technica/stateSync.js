...
exports.syncPublishedTechnicaContentState = syncPublishedTechnicaContentState;
const affinity_1 = require("../../core/affinity");
const campaign_1 = require("../../core/campaign");
const equipment_1 = require("../../core/equipment");
const inventoryIcons_1 = require("../../core/inventoryIcons");
...
function buildImportedInventoryItem(item, existing) {
    ...
    return {
        ...
        ...item,
        quantity,
        iconPath: (0, inventoryIcons_1.getInventoryIconPath)(item.iconPath ?? existing?.iconPath),
    };
}
...
function buildRuntimeUnit(unit, existing) {
    ...
    return {
        ...
        affinities: existing?.affinities ?? (0, affinity_1.createDefaultAffinities)(),
...
function syncDisabledUnits(state) {
    ...
    const collectIfDisabled = (unitId) => {
        if (typeof unitId === "string" && (0, index_1.isTechnicaContentDisabled)("unit", unitId)) {
...
function syncImportedItems(state) {
    const starterItems = (0, index_1.getAllImportedItems)().filter((item) => item.acquisition?.startsWithPlayer !== false);
...
function syncImportedKeyItemDefinitions(state) {
    const keyItems = (0, index_1.getAllImportedKeyItems)();
...
function syncImportedGear(state) {
    const importedGear = (0, index_1.getAllImportedGear)();
...
function syncImportedCards(state) {
    const importedCards = (0, index_1.getAllImportedCards)();
...
function syncImportedUnits(state) {
    const importedUnits = (0, index_1.getAllImportedUnits)().filter((unit) => unit.spawnRole !== "enemy" && (unit.startingInRoster !== false || unit.deployInParty));
...
function syncImportedMail(state) {
    const importedMailEntries = (0, index_1.getAllImportedMailEntries)();
...
function syncImportedCodex(state) {
    const importedCodexEntries = (0, index_1.getAllImportedCodexEntries)();
...
function syncPublishedTechnicaContentState(state, registryFingerprint) {
