...
exports.getResolvedBattleCard = getResolvedBattleCard;
exports.isChaosBattleCardId = isChaosBattleCardId;
exports.toCoreCard = toCoreCard;
...
function getCatalogCards() {
    return Object.values((0, equipment_1.getAllEquipmentCards)());
}
...
function toResolvedBattleCard(card) {
    ...
    const resolved = {
        ...
        description,
        effects,
        effectFlow: effects.length > 0 ? (0, effectFlow_1.createEffectFlowFromLegacyCardEffects)(effects, target === "ally" ? "ally" : target) : undefined,
...
function toResolvedImportedCard(card) {
    ...
    const resolved = {
        ...
        effectFlow: card.effectFlow ?? (card.effects.length > 0 ? (0, effectFlow_1.createEffectFlowFromLegacyCardEffects)(card.effects, card.targetType) : undefined),
        sourceEquipmentId,
        isChaosCard,
...
function getResolvedBattleCard(id) {
    const imported = (0, technica_1.getImportedCard)(id);
    if (imported) {
        return toResolvedImportedCard(imported);
    }
    if ((0, technica_1.isTechnicaContentDisabled)("card", id)) {
        return null;
    }
    ...
    const normalizedImported = (0, technica_1.getImportedCard)(normalized);
    if (normalizedImported) {
        return toResolvedImportedCard(normalizedImported);
    }
    if ((0, technica_1.isTechnicaContentDisabled)("card", normalized)) {
        return null;
    }
    return BATTLE_CARD_LOOKUP[normalized] || null;
}
function isChaosBattleCardId(id) {
    return Boolean(getResolvedBattleCard(id)?.isChaosCard);
}
function toCoreCard(card) {
    return {
        id: card.id,
        name: card.name,
        description: card.description,
