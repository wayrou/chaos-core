...
exports.COVER_STATS = void 0;
exports.generateCover = generateCover;
exports.getCoverDamageReduction = getCoverDamageReduction;
exports.getCoverVisualState = getCoverVisualState;
exports.damageCover = damageCover;
// ----------------------------------------------------------------------------
// COVER STATS
// ----------------------------------------------------------------------------
exports.COVER_STATS = {
 ...
 */
function generateCover(tiles, gridWidth, gridHeight, battleSeed, reservedCells = []) {
    ...
    if (DEBUG_COVER) {
        console.log(`[Cover] seed=${battleSeed.substring(0, 20)}..., ` +
    ...
    for (let i = 0; i < heavyCoverCount && positionIndex < shuffled.length; i++) {
        ...
        if (tileIndex >= 0 && tileIndex < updatedTiles.length) {
            const maxHp = rng.nextInt(exports.COVER_STATS.heavy_cover.minHp, exports.COVER_STATS.heavy_cover.maxHp);
    ...
    for (let i = 0; i < lightCoverCount && positionIndex < shuffled.length; i++) {
        ...
        if (tileIndex >= 0 && tileIndex < updatedTiles.length) {
            ...
            const maxHp = rng.nextInt(exports.COVER_STATS.light_cover.minHp, exports.COVER_STATS.light_cover.maxHp);
            updatedTiles[tileIndex] = {
                ...updatedTiles[tileIndex],
                terrain: "light_cover",
                cover: {
    ...
    if (DEBUG_COVER) {
        ...
        console.log(`[Cover] Generated: ${finalLightCount} light cover, ${finalHeavyCount} heavy cover`);
    }
    return updatedTiles;
}
/**
...
function getCoverDamageReduction(tile) {
    ...
    return exports.COVER_STATS[tile.cover.type].damageReduction;
}
/**
 * Get visual state of cover
 */
function getCoverVisualState(tile) {
 ...
 */
function damageCover(tile, damage) {
    if (!tile.cover || tile.cover.hp <= 0) {
        return tile;
    }
