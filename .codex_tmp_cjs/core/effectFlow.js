...
exports.normalizeEffectFlowDocument = normalizeEffectFlowDocument;
exports.createEffectFlowFromLegacyCardEffects = createEffectFlowFromLegacyCardEffects;
exports.createEffectFlowFromLegacyFieldModEffect = createEffectFlowFromLegacyFieldModEffect;
exports.applyEffectFlowToBattle = applyEffectFlowToBattle;
...
function normalizeEffectFlowDocument(value) {
...
function createEffectFlowFromLegacyCardEffects(effects, targetType) {
    if (effects.length === 0) {
        return { version: 1, entryNodeId: null, nodes: [], edges: [] };
    }
...
function createEffectFlowFromLegacyFieldModEffect(effect) {
    const node = (() => {
        switch (effect.kind) {
            case "deal_damage":
                return {
...
function applyDirectDamage(state, targetId, amount, sourceLabel) {
    ...
    if (newHp <= 0) {
        ...
        next = (0, battle_1.appendBattleLog)(next, `SLK//FLOW  :: ${sourceLabel} defeats ${target.name} for ${amount} damage.`);
    }
    else {
        next = {
            ...state,
        ...
        next = (0, battle_1.appendBattleLog)(next, `SLK//FLOW  :: ${sourceLabel} deals ${amount} damage to ${target.name}.`);
    }
    return (0, battle_1.evaluateBattleOutcome)(next);
}
...
function evaluateCondition(state, node, currentSelection, context) {
    ...
    switch (node.condition) {
        ...
        case "target_has_status": {
            ...
            return status ? selection.unitIds.some((unitId) => state.units[unitId] && (0, battle_1.hasStatus)(state.units[unitId], status)) : false;
        }
        case "target_missing_status": {
            const status = statusToBattleStatus(node.status);
            return status ? selection.unitIds.some((unitId) => state.units[unitId] && !(0, battle_1.hasStatus)(state.units[unitId], status)) : false;
        ...
        case "source_has_status": {
            const status = statusToBattleStatus(node.status);
            return sourceUnit && status ? (0, battle_1.hasStatus)(sourceUnit, status) : false;
        }
        case "source_missing_status": {
            const status = statusToBattleStatus(node.status);
            return sourceUnit && status ? !(0, battle_1.hasStatus)(sourceUnit, status) : false;
...
function moveUnitTo(state, unitId, destination, sourceLabel) {
    ...
    if (hasLivingUnitAt(state, clamped, unitId)) {
        return (0, battle_1.appendBattleLog)(state, `SLK//FLOW  :: ${sourceLabel} could not move ${unit.name}; destination blocked.`);
    }
    return (0, battle_1.appendBattleLog)({
        ...state,
        units: {
            ...state.units,
            [unitId]: {
    ...
    switch (node.action) {
        ...
        case "grant_shield": {
            ...
            selection.unitIds.forEach((unitId) => {
                ...
                next = (0, battle_1.appendBattleLog)(next, `SLK//FLOW  :: ${sourceLabel} ${node.action === "heal" ? "heals" : "grants shield to"} ${unit.name} for ${amount}.`);
            });
            return next;
        }
        case "draw_cards": {
            ...
            selection.unitIds.forEach((unitId) => {
                ...
                const unit = next.units[unitId];
                if (unit) {
                    next = (0, battle_1.appendBattleLog)(next, `SLK//FLOW  :: ${sourceLabel} draws ${amount} card(s) for ${unit.name}.`);
                }
            });
            return next;
        }
        case "draw_until_hand_size": {
            ...
            selection.unitIds.forEach((unitId) => {
                ...
                if (updatedUnit && cardsNeeded > 0) {
                    next = (0, battle_1.appendBattleLog)(next, `SLK//FLOW  :: ${sourceLabel} draws up to ${targetHandSize} cards for ${updatedUnit.name}.`);
                }
            });
            return next;
        }
        case "modify_stat": {
            ...
            selection.unitIds.forEach((unitId) => {
                ...
                next = (0, battle_1.appendBattleLog)(next, `SLK//FLOW  :: ${sourceLabel} ${signedAmount < 0 ? "reduces" : "boosts"} ${unit.name}'s ${stat.toUpperCase()} by ${Math.abs(signedAmount)}.`);
            });
            return next;
        }
        case "apply_status": {
            ...
            const status = statusToBattleStatus(node.status);
            if (!status) {
                return (0, battle_1.appendBattleLog)(next, `SLK//FLOW  :: ${sourceLabel} references unsupported status '${node.status ?? "unknown"}'.`);
            }
            selection.unitIds.forEach((unitId) => {
                next = (0, battle_1.addStatus)(next, unitId, status, duration, context.sourceUnitId ?? undefined);
                const unit = next.units[unitId];
                if (unit) {
                    next = (0, battle_1.appendBattleLog)(next, `SLK//FLOW  :: ${sourceLabel} applies ${status.toUpperCase()} to ${unit.name}.`);
                }
            });
            return next;
        }
        case "move_target": {
            if (selection.unitIds.length === 0 || !context.selectedTilePos) {
                return (0, battle_1.appendBattleLog)(next, `SLK//FLOW  :: ${sourceLabel} had no destination tile for move.`);
            }
        ...
        case "end_turn": {
            ...
            if (targetUnitId && next.activeUnitId === targetUnitId) {
                return (0, battle_1.appendBattleLog)((0, battle_1.advanceTurn)(next), `SLK//FLOW  :: ${sourceLabel} ends the turn.`);
            }
            return (0, battle_1.appendBattleLog)(next, `SLK//FLOW  :: ${sourceLabel} tried to end a non-active turn.`);
        }
        case "set_flag":
            return (0, battle_1.appendBattleLog)(next, `SLK//FLOW  :: ${sourceLabel} sets flag ${node.flagKey ?? "flag"} = ${node.flagValue ?? "true"}.`);
        case "reduce_cost_next_card":
            return (0, battle_1.appendBattleLog)(next, `SLK//FLOW  :: ${sourceLabel} reduces the next card cost by ${scaleValue(node.amount, multiplier) ?? 0}.`);
        case "discard_cards": {
            const amount = scaleValue(node.amount, multiplier) ?? 0;
            selection.unitIds.forEach((unitId) => {
                ...
                const unit = next.units[unitId];
                if (unit) {
                    next = (0, battle_1.appendBattleLog)(next, `SLK//FLOW  :: ${sourceLabel} discards ${amount} card(s) from ${unit.name}.`);
                }
            });
            return next;
        }
        case "exhaust_cards": {
            ...
            selection.unitIds.forEach((unitId) => {
                ...
                const unit = next.units[unitId];
                if (unit) {
                    next = (0, battle_1.appendBattleLog)(next, `SLK//FLOW  :: ${sourceLabel} exhausts ${amount} card(s) from ${unit.name}.`);
                }
            });
            return next;
        }
        case "restore_strain": {
            ...
            selection.unitIds.forEach((unitId) => {
                ...
                const unit = next.units[unitId];
                if (unit) {
                    next = (0, battle_1.appendBattleLog)(next, `SLK//FLOW  :: ${sourceLabel} restores ${amount} strain on ${unit.name}.`);
                }
            });
            return next;
        }
        case "cleanse_statuses": {
            selection.unitIds.forEach((unitId) => {
                ...
                const unit = next.units[unitId];
                if (unit) {
                    next = (0, battle_1.appendBattleLog)(next, `SLK//FLOW  :: ${sourceLabel} ${node.status ? `cleanses ${node.status}` : "cleanses statuses"} from ${unit.name}.`);
                }
            });
            return next;
        }
        case "silence_buffs": {
            selection.unitIds.forEach((unitId) => {
                ...
                const unit = next.units[unitId];
                if (unit) {
                    next = (0, battle_1.appendBattleLog)(next, `SLK//FLOW  :: ${sourceLabel} silences buffs on ${unit.name}.`);
                }
            });
            return next;
        }
        case "gain_resource":
            return (0, battle_1.appendBattleLog)(next, `SLK//FLOW  :: ${sourceLabel} grants ${scaleValue(node.amount, multiplier) ?? 0} ${node.resource ?? "resource"}.`);
        case "summon_drone":
            return (0, battle_1.appendBattleLog)(next, `SLK//FLOW  :: ${sourceLabel} summons ${scaleValue(node.count, multiplier) ?? 0} ${node.droneTypeId ?? "drone"} unit(s).`);
    }
}
function applyEffectFlowToBattle(state, flowDocument, context) {
