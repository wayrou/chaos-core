...
exports.generateElevationMap = generateElevationMap;
exports.generateStructuredBoardLayout = generateStructuredBoardLayout;
...
function relaxElevationMap(grid, passes) {
...
function flattenSpawnEdges(grid) {
    const width = grid.length;
    const height = grid[0]?.length ?? 0;
    if (width <= 0 || height <= 0) {
        return;
...
function generateStructuredBoardLayout(width, height, seed, profile = "encounter") {
    ...
    for (let featureIndex = 0; featureIndex < featureCount; featureIndex += 1) {
        ...
        for (let x = startX; x < startX + plateauWidth; x += 1) {
            for (let y = startY; y < startY + plateauHeight; y += 1) {
                const edgeDistance = Math.min(x - startX, startX + plateauWidth - 1 - x, y - startY, startY + plateauHeight - 1 - y);
    ...
    const baseSurface = rng.pick(profile === "defense"
        ? ["stone", "metal", "industrial"]
        : ["dirt", "grate", "industrial", "stone"]);
    const highSurface = profile === "defense" ? "stone" : rng.pick(["stone", "ruin"]);
    const lowSurface = profile === "defense" ? "metal" : rng.pick(["dirt", "grate"]);
    const surfaces = {};
    for (let x = 0; x < width; x += 1) {
        for (let y = 0; y < height; y += 1) {
