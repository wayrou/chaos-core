...
exports.OPS_ATLAS_HAVEN_ANCHOR = exports.OPS_ATLAS_MAP_HEIGHT = exports.OPS_ATLAS_MAP_WIDTH = void 0;
exports.loadOpsTerminalAtlasProgress = loadOpsTerminalAtlasProgress;
exports.getCurrentOpsTerminalAtlasFloor = getCurrentOpsTerminalAtlasFloor;
exports.getOpsTerminalAtlasWarmEconomySummaries = getOpsTerminalAtlasWarmEconomySummaries;
exports.getOpsTerminalAtlasOtherWarmEconomySummaries = getOpsTerminalAtlasOtherWarmEconomySummaries;
exports.getHighestGeneratedOpsTerminalAtlasFloorOrdinal = getHighestGeneratedOpsTerminalAtlasFloorOrdinal;
exports.setCurrentOpsTerminalAtlasFloorOrdinal = setCurrentOpsTerminalAtlasFloorOrdinal;
exports.regenerateOpsTerminalAtlasFloor = regenerateOpsTerminalAtlasFloor;
exports.restartOpsTerminalAtlas = restartOpsTerminalAtlas;
exports.applyWarmTheaterEconomyToState = applyWarmTheaterEconomyToState;
exports.holdPositionInOpsTerminalAtlas = holdPositionInOpsTerminalAtlas;
exports.getOpsTerminalAtlasSectorState = getOpsTerminalAtlasSectorState;
exports.createOpsTerminalSectorOperation = createOpsTerminalSectorOperation;
exports.isOpsTerminalAtlasOperation = isOpsTerminalAtlasOperation;
exports.syncOpsTerminalOperationState = syncOpsTerminalOperationState;
exports.getSectorSecuredRoomCount = getSectorSecuredRoomCount;
exports.getSectorKnownRoomCount = getSectorKnownRoomCount;
exports.getSectorStateLabel = getSectorStateLabel;
exports.getSectorCoreCount = getSectorCoreCount;
exports.getOpsTerminalAtlasCoreSummaries = getOpsTerminalAtlasCoreSummaries;
const campaign_1 = require("./campaign");
const campaignRegions_1 = require("./campaignRegions");
const theaterSystem_1 = require("./theaterSystem");
const resources_1 = require("./resources");
const session_1 = require("./session");
exports.OPS_ATLAS_MAP_WIDTH = 6200;
exports.OPS_ATLAS_MAP_HEIGHT = 6200;
exports.OPS_ATLAS_HAVEN_ANCHOR = { x: 3100, y: 3100 };
...
const KEY_TYPES = ["triangle", "square", "circle", "spade", "star"];
...
function cloneTheater(theater) {
    return JSON.parse(JSON.stringify(theater));
}
...
function cloneAtlasState(state) {
    return {
        ...state,
        floorsById: Object.fromEntries(Object.entries(state.floorsById).map(([floorId, floor]) => [floorId, cloneFloor(floor)])),
    };
}
function addResourceWallet(base, delta) {
    return (0, resources_1.addResourceWallet)(base, delta);
}
function scaleResourceWallet(wallet, ticks) {
    const scaled = (0, resources_1.createEmptyResourceWallet)();
    resources_1.RESOURCE_KEYS.forEach((key) => {
        scaled[key] = (wallet[key] ?? 0) * ticks;
    });
    return scaled;
}
function hasPositiveResourceGain(wallet) {
    return resources_1.RESOURCE_KEYS.some((key) => Number(wallet?.[key] ?? 0) > 0);
}
...
function mergeKeyInventories(...inventories) {
    return inventories.reduce((merged, inventory) => ({
...
function applyFloorKeyInventoryToSector(sector, keyInventory) {
    sector.theater.definition.floorKeyInventory = cloneKeyInventory(keyInventory);
}
function syncFloorKeyInventoryToSectors(floor) {
...
function formatFloorLabel(floorOrdinal) {
    const presentation = (0, campaignRegions_1.getActiveRegionPresentation)(floorOrdinal);
    return `FLOOR ${String(floorOrdinal).padStart(2, "0")} // ${presentation.regionName.toUpperCase()} SURVEY`;
}
function resolveSectorState(atlas, sector) {
...
function applyPresentationStateToSector(atlas, sector) {
...
function decorateAtlasState(atlas) {
...
function createEconomySummary(atlas, sector) {
    const economy = (0, theaterSystem_1.getTheaterUpkeepPerTick)(sector.theater);
    return {
        ...
        incomePerTick: (0, resources_1.createEmptyResourceWallet)(economy.incomePerTick),
    };
}
function createCoreSummary(atlas, sector, room) {
    ...
    return {
        ...
        currentState: resolveSectorState(atlas, sector),
        operational: (0, theaterSystem_1.isTheaterCoreOperational)(room),
        offlineReason: (0, theaterSystem_1.getTheaterCoreOfflineReason)(room),
        wadUpkeepPerTick: coreAssignment.wadUpkeepPerTick ?? 0,
        incomePerTick: (0, resources_1.createEmptyResourceWallet)(coreAssignment.incomePerTick),
        supplyFlow: room.supplyFlow ?? 0,
        powerFlow: room.powerFlow ?? 0,
        commsFlow: room.commsFlow ?? 0,
    };
}
function hasEconomyActivity(summary) {
    return (summary.wadUpkeepPerTick > 0
        || resources_1.RESOURCE_KEYS.some((key) => summary.incomePerTick[key] > 0));
}
function isOperationalEconomyState(state) {
    return state === "warm" || state === "active";
}
function findSectorInAtlas(atlas, theaterId) {
...
function chooseLockRoomIdOnObjectivePath(theater, path, usedRoomIds, rng) {
...
function chooseKeyRoomId(theater, path, lockRoomId, usedRoomIds, rng) {
...
function assignFloorLocksAndKeys(floor, rng) {
    ...
    const usedKeyRoomIds = new Set();
    const usedLockRoomIds = new Set();
    const lockRoomIdBySectorIndex = new Map();
...
function countActiveCores(theater) {
    return Object.values(theater.rooms).filter((room) => room.coreAssignment).length;
}
function buildSectorOperationPrototype(floorOrdinal, floorId, sectorIndex, zoneName, codename, description, sprawlDirection, seed) {
    return {
...
function buildSectorTheater(floorOrdinal, floorId, sectorIndex, sprawlDirection, seed, rng) {
    const regionPresentation = (0, campaignRegions_1.getActiveRegionPresentation)(floorOrdinal);
    ...
    const zoneName = (0, campaignRegions_1.buildCampaignRegionZoneName)(floorOrdinal, rng.pick);
    ...
    const prototype = buildSectorOperationPrototype(floorOrdinal, floorId, sectorIndex, zoneName, codename, description, sprawlDirection, seed);
    const generatedOperation = (0, theaterSystem_1.ensureOperationHasTheater)(prototype);
    ...
    theater.rooms = Object.fromEntries(Object.entries(theater.rooms).map(([roomId, room]) => [
        roomId,
        {
            ...room,
            theaterId: sectorId,
...
function createFloorState(seed, floorOrdinal) {
    ...
    return assignFloorLocksAndKeys({
        ...
        sectors: DIRECTION_ORDER.map((direction, sectorIndex) => (buildSectorTheater(floorOrdinal, floorId, sectorIndex, direction, `${floorSeed}:${sectorIndex}`, createSeededRng(`${floorSeed}:${sectorIndex}:${direction}:${rng.nextInt(0, 99999)}`)))),
    }, rng);
}
function ensureFloorGenerated(atlas, floorOrdinal) {
    const floorId = getFloorId(floorOrdinal);
...
function ensureCurrentFloorInitialized(progress) {
    const existingAtlas = progress.opsTerminalAtlas;
    const needsRegeneration = Boolean(existingAtlas) && existingAtlas?.generationVersion !== OPS_ATLAS_GENERATION_VERSION;
    const highestKnownFloorOrdinal = existingAtlas
        ? Math.max(existingAtlas.currentFloorOrdinal, ...Object.values(existingAtlas.floorsById).map((floor) => floor.floorOrdinal))
        : 1;
    let baseAtlas = existingAtlas && !needsRegeneration
    ...
    const atlasWithCurrentFloor = decorateAtlasState(ensureFloorGenerated(baseAtlas, Math.max(1, baseAtlas.currentFloorOrdinal)));
...
function updateCurrentFloorIfCleared(progress) {
...
function loadOpsTerminalAtlasProgress() {
    ...
    if (initialized !== progress) {
        (0, campaign_1.saveCampaignProgress)(initialized);
    }
    return initialized;
}
function getCurrentOpsTerminalAtlasFloor(progress = loadOpsTerminalAtlasProgress()) {
...
function getOpsTerminalAtlasWarmEconomySummaries(floorOrdinal, progress = loadOpsTerminalAtlasProgress()) {
    const ensured = ensureCurrentFloorInitialized(progress);
    const atlas = decorateAtlasState(ensured.opsTerminalAtlas);
...
function getOpsTerminalAtlasOtherWarmEconomySummaries(activeTheaterId, progress = loadOpsTerminalAtlasProgress()) {
    const ensured = ensureCurrentFloorInitialized(progress);
    const atlas = decorateAtlasState(ensured.opsTerminalAtlas);
...
function getHighestGeneratedOpsTerminalAtlasFloorOrdinal(progress = loadOpsTerminalAtlasProgress()) {
    const ensured = ensureCurrentFloorInitialized(progress);
    const atlas = ensured.opsTerminalAtlas;
    if (!atlas) {
        return 1;
    }
    return Math.max(atlas.currentFloorOrdinal, ...Object.values(atlas.floorsById).map((floor) => floor.floorOrdinal));
}
function setCurrentOpsTerminalAtlasFloorOrdinal(floorOrdinal, progress = loadOpsTerminalAtlasProgress(), allowObjectiveBypass = false) {
    const ensured = ensureCurrentFloorInitialized(progress);
    ...
    (0, campaign_1.saveCampaignProgress)({
        ...ensured,
...
function regenerateOpsTerminalAtlasFloor(floorOrdinal, progress = loadOpsTerminalAtlasProgress()) {
    const ensured = ensureCurrentFloorInitialized(progress);
    const atlas = ensured.opsTerminalAtlas ? cloneAtlasState(ensured.opsTerminalAtlas) : null;
    if (!atlas) {
        throw new Error("Ops terminal atlas state is not initialized.");
...
function restartOpsTerminalAtlas(progress = loadOpsTerminalAtlasProgress()) {
    const ensured = ensureCurrentFloorInitialized(progress);
...
function applyWarmTheaterEconomyToState(state, activeTheaterId, tickCost) {
    if (tickCost <= 0) {
        return state;
    }
    ...
    const playerSlot = (0, session_1.getLocalSessionPlayerSlot)(state);
    let availableWad = (0, session_1.getSessionResourcePool)(state, playerSlot).wad;
    let totalWadSpent = 0;
    let totalIncomeGain = (0, resources_1.createEmptyResourceWallet)();
    let mutated = false;
    located.floor.sectors = located.floor.sectors.map((sector) => {
        ...
        const economy = (0, theaterSystem_1.getTheaterUpkeepPerTick)(nextSector.theater);
        if (economy.wadUpkeep <= 0
    ...
    let nextState = state;
    if (totalWadSpent > 0) {
        const spendResult = (0, session_1.spendSessionCost)(nextState, { wad: totalWadSpent }, playerSlot);
        if (spendResult.success) {
            nextState = spendResult.state;
        }
    }
    if (hasPositiveResourceGain(totalIncomeGain)) {
        nextState = (0, session_1.grantSessionResources)(nextState, { resources: totalIncomeGain }, playerSlot);
    }
    return nextState;
}
function holdPositionInOpsTerminalAtlas(state, floorOrdinal, ticks = 1) {
...
function getOpsTerminalAtlasSectorState(theaterId, progress = loadOpsTerminalAtlasProgress()) {
...
function createOpsTerminalSectorOperation(theaterId) {
    const progress = loadOpsTerminalAtlasProgress();
...
function isOpsTerminalAtlasOperation(operation) {
    return Boolean(operation?.launchSource === "ops_terminal"
        && typeof operation.atlasTheaterId === "string"
        && operation.atlasTheaterId.startsWith(OPS_ATLAS_SECTOR_PREFIX));
}
function syncOpsTerminalOperationState(operation) {
    ...
    const mergedFloorKeyInventory = mergeKeyInventories(floor.keyInventory, nextSector.theater.definition.floorKeyInventory);
    ...
    nextSector.theater.rooms = Object.fromEntries(Object.entries(nextSector.theater.rooms).map(([roomId, room]) => [
        roomId,
        {
            ...room,
    ...
    if (nextSector.linkedOperationId
        && nextSector.theater.objectiveComplete
        && !updatedProgress.completedOperations.includes(nextSector.linkedOperationId)) {
        updatedProgress = (0, campaign_1.completeOperation)(nextSector.linkedOperationId, updatedProgress);
    }
    (0, campaign_1.saveCampaignProgress)(updatedProgress);
    return true;
}
function getSectorSecuredRoomCount(sector) {
    return Object.values(sector.theater.rooms).filter((room) => room.secured).length;
}
function getSectorKnownRoomCount(sector) {
    return Object.values(sector.theater.rooms).length;
}
function getSectorStateLabel(sector) {
    return sector.theater.definition.currentState;
}
function getSectorCoreCount(sector) {
    return countActiveCores(sector.theater);
}
function getOpsTerminalAtlasCoreSummaries(floorOrdinal, progress = loadOpsTerminalAtlasProgress()) {
    const ensured = ensureCurrentFloorInitialized(progress);
    const atlas = decorateAtlasState(ensured.opsTerminalAtlas);
    const resolvedFloorOrdinal = floorOrdinal ?? atlas.currentFloorOrdinal;
    const floor = atlas.floorsById[getFloorId(resolvedFloorOrdinal)];
    ...
    return floor.sectors.flatMap((sector) => (Object.values(sector.theater.rooms)
        .filter((room) => room.coreAssignment)
        .map((room) => createCoreSummary(atlas, sector, room)))).sort((left, right) => (left.sectorLabel.localeCompare(right.sectorLabel)
