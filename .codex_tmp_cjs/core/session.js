...
exports.getBattleRuntimeContext = getBattleRuntimeContext;
exports.getBattleStateById = getBattleStateById;
exports.getMountedOrActiveBattleState = getMountedOrActiveBattleState;
exports.mountBattleState = mountBattleState;
exports.mountBattleContextById = mountBattleContextById;
exports.replaceBattleStateById = replaceBattleStateById;
exports.createDefaultSessionState = createDefaultSessionState;
exports.withNormalizedSessionState = withNormalizedSessionState;
exports.getPlayerControllerLabel = getPlayerControllerLabel;
exports.getUnitOwnerLabel = getUnitOwnerLabel;
exports.getPlayerColor = getPlayerColor;
exports.isLocalCoopActive = isLocalCoopActive;
exports.canPlayerUseFieldAction = canPlayerUseFieldAction;
exports.getFieldActionRestrictionMessage = getFieldActionRestrictionMessage;
exports.setPlayerJoinState = setPlayerJoinState;
exports.getTheaterAssignedPlayerSlots = getTheaterAssignedPlayerSlots;
exports.assignLocalPlayerToTheaterSquad = assignLocalPlayerToTheaterSquad;
exports.launchCoopOperationsSessionFromLobby = launchCoopOperationsSessionFromLobby;
exports.clearCoopOperationsSession = clearCoopOperationsSession;
exports.getSharedEconomyPreset = getSharedEconomyPreset;
exports.getSessionEconomyParticipantSlots = getSessionEconomyParticipantSlots;
exports.getLocalSessionPlayerSlot = getLocalSessionPlayerSlot;
exports.getSessionResourcePool = getSessionResourcePool;
exports.canSessionAffordCost = canSessionAffordCost;
exports.spendSessionCost = spendSessionCost;
exports.grantSessionResources = grantSessionResources;
exports.setSharedEconomyPreset = setSharedEconomyPreset;
exports.requestSessionTradeTransfer = requestSessionTradeTransfer;
exports.approveSessionTradeTransfer = approveSessionTradeTransfer;
exports.cancelSessionTradeTransfer = cancelSessionTradeTransfer;
exports.describeSessionTradeTransfer = describeSessionTradeTransfer;
const types_1 = require("./types");
const resources_1 = require("./resources");
const EMPTY_RESOURCE_WALLET = (0, resources_1.createEmptyResourceWallet)();
...
function cloneResourceWallet(wallet) {
    return (0, resources_1.createEmptyResourceWallet)(wallet);
}
...
function sumResourcePools(pools) {
    let wad = 0;
    let resources = (0, resources_1.createEmptyResourceWallet)();
    for (const pool of pools) {
        wad += Number(pool?.wad ?? 0);
        resources = (0, resources_1.addResourceWallet)(resources, pool?.resources);
    }
    return createResourcePool(wad, resources);
}
function getEconomyParticipantSlotsFromSession(session) {
    const activeSlots = types_1.SESSION_PLAYER_SLOTS.filter((slot) => {
...
function getLocalSessionPlayerSlotFromSession(session) {
    const localSlot = types_1.SESSION_PLAYER_SLOTS.find((slot) => {
        const player = session.players?.[slot];
...
function distributeIntegerEvenly(amount, slots) {
    const next = types_1.SESSION_PLAYER_SLOTS.reduce((acc, slot) => {
        acc[slot] = 0;
        return acc;
    }, {});
...
function distributeResourcePoolEvenly(pool, slots) {
    ...
    for (const key of resources_1.RESOURCE_KEYS) {
...
function hasAnyPartitionedAllocations(perPlayer, slots) {
    return slots.some((slot) => {
        ...
        return pool.wad > 0 || resources_1.RESOURCE_KEYS.some((key) => Number(pool.resources[key] ?? 0) > 0);
    });
}
...
function getBattleRuntimeContext(state, battleId) {
    if (!battleId) {
        return null;
    }
    return state.session.activeBattleContexts?.[battleId] ?? null;
}
function getBattleStateById(state, battleId) {
    if (!battleId) {
        return null;
    }
...
function getMountedOrActiveBattleState(state) {
    return state.currentBattle
        ?? getBattleStateById(state, state.session.activeBattleId ?? null);
}
function mountBattleState(state, battle) {
    if (!battle) {
        return {
            ...state,
            currentBattle: null,
...
function mountBattleContextById(state, battleId) {
    const battle = getBattleStateById(state, battleId);
    return battle ? mountBattleState(state, battle) : state;
}
function replaceBattleStateById(state, battleId, battle) {
...
function getCoopParticipantNetworkSlot(lobby, sessionSlot) {
    ...
    const match = types_1.NETWORK_PLAYER_SLOTS.find((slot) => coopOperations.participants[slot]?.selected
        && coopOperations.participants[slot]?.sessionSlot === sessionSlot);
    return match ?? null;
}
function createDefaultSessionState(seed) {
    ...
    return {
        ...
        pendingTransfers: [],
        players: types_1.SESSION_PLAYER_SLOTS.reduce((acc, slot) => {
            if (slot === "P1") {
        ...
        theaterAssignments: types_1.SESSION_PLAYER_SLOTS.reduce((acc, slot) => {
...
function withNormalizedSessionState(state) {
    ...
    const nextAssignments = types_1.SESSION_PLAYER_SLOTS.reduce((acc, slot) => {
        acc[slot] = normalizeTheaterAssignment(state, slot, currentSession.theaterAssignments?.[slot]);
        return acc;
    }, {});
    const nextPlayers = types_1.SESSION_PLAYER_SLOTS.reduce((acc, slot) => {
...
function getPlayerControllerLabel(playerId) {
    return `PLAYER ${playerId.replace("P", "")}`;
}
function getUnitOwnerLabel(controller) {
...
function isLocalCoopActive(state) {
    return state.session.mode === "local_coop" || state.players.P2.active;
}
function canPlayerUseFieldAction(state, playerId, action) {
    if (playerId === "P1") {
        return true;
    }
...
function getFieldActionRestrictionMessage(actionLabel) {
    return `Player 1 must authorize ${actionLabel}.`;
}
function setPlayerJoinState(state, playerId, active) {
    if (playerId !== "P2") {
        return state;
    }
...
function getTheaterAssignedPlayerSlots(state, squadId) {
    if (!squadId) {
        return [];
    }
    return types_1.LOCAL_PLAYER_IDS.filter((slot) => state.players[slot].active && state.session.theaterAssignments?.[slot]?.squadId === squadId);
}
function assignLocalPlayerToTheaterSquad(state, playerId, squadId) {
...
function launchCoopOperationsSessionFromLobby(state, lobby) {
    ...
    const nextPlayers = types_1.SESSION_PLAYER_SLOTS.reduce((acc, sessionSlot) => {
    ...
    const nextAssignments = types_1.SESSION_PLAYER_SLOTS.reduce((acc, sessionSlot) => {
        const participant = (() => {
            const networkSlot = getCoopParticipantNetworkSlot(lobby, sessionSlot);
    ...
    return withNormalizedSessionState({
        ...
        session: {
            ...
            ownerSlot: "P1",
            maxPlayers: Math.max(2, Math.min(types_1.SESSION_PLAYER_SLOTS.length, activity.selectedSlots.length || 1)),
...
function clearCoopOperationsSession(state) {
    if (state.session.mode !== "coop_operations") {
        return state;
    }
...
function canSessionAffordCost(state, cost, playerId) {
    const { pool } = resolveSessionEconomyPool(state, playerId);
    const wadCost = Math.max(0, Math.floor(cost.wad ?? 0));
    return pool.wad >= wadCost && (0, resources_1.hasEnoughResources)(pool.resources, cost.resources);
}
function spendSessionCost(state, cost, playerId) {
    const currentSession = state.session ?? createDefaultSessionState(state);
    ...
    const wadCost = Math.max(0, Math.floor(cost.wad ?? 0));
    const normalizedResourceCost = (0, resources_1.createEmptyResourceWallet)(cost.resources);
    const targetPool = ledger.preset === "shared"
        ? cloneResourcePool(ledger.shared)
        : cloneResourcePool(ledger.perPlayer[resolvedPlayerId]);
    if (targetPool.wad < wadCost || !(0, resources_1.hasEnoughResources)(targetPool.resources, normalizedResourceCost)) {
        return {
            success: false,
            state,
            playerId: resolvedPlayerId,
    ...
    const nextTargetPool = {
        ...
        resources: (0, resources_1.subtractResourceWallet)(targetPool.resources, normalizedResourceCost, true),
    };
...
function grantSessionResources(state, reward, playerId) {
    const currentSession = state.session ?? createDefaultSessionState(state);
    const ledger = cloneResourceLedger(currentSession.resourceLedger);
    const resolvedPlayerId = playerId ?? getLocalSessionPlayerSlotFromSession(currentSession);
    const wadGain = Math.max(0, Math.floor(reward.wad ?? 0));
    const normalizedResourceGain = (0, resources_1.createEmptyResourceWallet)(reward.resources);
    if (ledger.preset === "shared") {
        ledger.shared = {
            wad: ledger.shared.wad + wadGain,
            resources: (0, resources_1.addResourceWallet)(ledger.shared.resources, normalizedResourceGain),
        };
    }
    else {
        ledger.perPlayer[resolvedPlayerId] = {
            wad: ledger.perPlayer[resolvedPlayerId].wad + wadGain,
            resources: (0, resources_1.addResourceWallet)(ledger.perPlayer[resolvedPlayerId].resources, normalizedResourceGain),
        };
        ledger.shared = sumResourcePools(getEconomyParticipantSlotsFromSession(currentSession).map((slot) => ledger.perPlayer[slot]));
    }
    return withNormalizedSessionState({
...
function setSharedEconomyPreset(state, preset) {
    const currentSession = state.session ?? createDefaultSessionState(state);
...
function requestSessionTradeTransfer(state, request) {
...
function approveSessionTradeTransfer(state, transferId) {
    const currentSession = state.session ?? createDefaultSessionState(state);
    if (currentSession.resourceLedger.preset !== "partitioned") {
        return state;
    }
    ...
    if (currentTransfer.kind === "wad") {
        ...
        canComplete = resourceAmount > 0 && (0, resources_1.hasEnoughResources)(fromPool.resources, {
            [currentTransfer.resourceKey]: resourceAmount,
        });
        if (canComplete) {
            fromPool.resources = (0, resources_1.subtractResourceWallet)(fromPool.resources, {
                [currentTransfer.resourceKey]: resourceAmount,
            }, true);
            toPool.resources = (0, resources_1.addResourceWallet)(toPool.resources, {
                [currentTransfer.resourceKey]: resourceAmount,
            });
        }
    }
...
function cancelSessionTradeTransfer(state, transferId) {
    const currentSession = state.session ?? createDefaultSessionState(state);
...
function describeSessionTradeTransfer(transfer) {
