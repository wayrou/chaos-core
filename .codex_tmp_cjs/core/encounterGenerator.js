 ...
 */
function generateEncounter(nodeType, floorIndex, operationId, difficulty, rngSeed, context) {
    ...
    const pool = (0, enemies_1.getEnemyPool)(operationId);
    ...
    const maxEnemies = Math.min(maxUnitsPerSide, config.maxBase + difficultyMod + floorMod + config.variance);
    ...
    const enemyUnits = generateEnemyComposition(pool, targetCount, floorIndex, difficulty, rng);
    // Generate intro text
    ...
    if (DEBUG_ENCOUNTERS) {
        ...
        console.log(`[Encounter] node=${nodeType}, floor=${floorIndex}, op=${operationId}, ` +
 ...
 */
function generateEnemyComposition(pool, targetCount, floorIndex, difficulty, rng) {
    const composition = [];
    const floorOrdinal = floorIndex + 1;
    const importedEnemies = shuffleWithRng((0, technica_1.getImportedEncounterUnitsForFloorOrdinal)(floorOrdinal), rng);
    ...
    if (DEBUG_ENCOUNTERS) {
        console.log(`[Encounter:Comp] targetCount=${targetCount}, imported=${composition.length}, starting composition generation`);
    }
    // Filter enemies by floor constraints
    const availableEnemies = pool.enemies.filter(e => {
        if (e.minFloor !== undefined && floorIndex < e.minFloor)
            return false;
        if (e.maxFloor !== undefined && floorIndex > e.maxFloor)
            return false;
        return true;
    });
    if (DEBUG_ENCOUNTERS) {
 ...
 */
function determineGridSize(floorIndex, difficulty, nodeType, rng) {
 ...
 */
function shouldBeElite(floorIndex, difficulty, rng) {
 ...
 */
function generateIntroText(nodeType, enemyCount, rng) {
