...
exports.CAMPAIGN_REGION_CONFIGS = exports.MAX_CAMPAIGN_FLOOR_ORDINAL = exports.CAMPAIGN_REGION_COUNT = exports.CAMPAIGN_REGION_FLOOR_COUNT = void 0;
exports.clampCampaignFloorOrdinal = clampCampaignFloorOrdinal;
exports.getRegionForFloor = getRegionForFloor;
exports.getFloorRegionVariant = getFloorRegionVariant;
exports.getCampaignRegionConfig = getCampaignRegionConfig;
exports.getActiveRegionPresentation = getActiveRegionPresentation;
exports.buildCampaignRegionZoneName = buildCampaignRegionZoneName;
exports.CAMPAIGN_REGION_FLOOR_COUNT = 3;
exports.CAMPAIGN_REGION_COUNT = 4;
exports.MAX_CAMPAIGN_FLOOR_ORDINAL = exports.CAMPAIGN_REGION_FLOOR_COUNT * exports.CAMPAIGN_REGION_COUNT;
...
exports.CAMPAIGN_REGION_CONFIGS = {
    industrial: INDUSTRIAL_REGION,
    overgrowth: OVERGROWTH_REGION,
    flooded: FLOODED_REGION,
    chaos: CHAOS_REGION,
};
function clampCampaignFloorOrdinal(floorOrdinal) {
    const numericFloor = Math.max(1, Math.floor(floorOrdinal || 1));
    return Math.min(exports.MAX_CAMPAIGN_FLOOR_ORDINAL, numericFloor);
}
function getRegionForFloor(floorOrdinal) {
    const clampedFloor = clampCampaignFloorOrdinal(floorOrdinal);
    const regionIndex = Math.floor((clampedFloor - 1) / exports.CAMPAIGN_REGION_FLOOR_COUNT);
    return CAMPAIGN_REGION_ORDER[regionIndex] ?? "industrial";
}
function getFloorRegionVariant(floorOrdinal) {
    const clampedFloor = clampCampaignFloorOrdinal(floorOrdinal);
    const floorInRegion = ((clampedFloor - 1) % exports.CAMPAIGN_REGION_FLOOR_COUNT);
    return CAMPAIGN_VARIANT_ORDER[floorInRegion] ?? "teach";
}
function getCampaignRegionConfig(regionId) {
    return exports.CAMPAIGN_REGION_CONFIGS[regionId] ?? INDUSTRIAL_REGION;
}
function getActiveRegionPresentation(floorOrdinal) {
    const clampedFloor = clampCampaignFloorOrdinal(floorOrdinal);
    const regionId = getRegionForFloor(clampedFloor);
    const regionConfig = getCampaignRegionConfig(regionId);
    const floorInRegion = (((clampedFloor - 1) % exports.CAMPAIGN_REGION_FLOOR_COUNT) + 1);
    const floorConfig = regionConfig.floors[floorInRegion];
    return {
        ...floorConfig,
        floorOrdinal: clampedFloor,
        floorInRegion,
        regionIndex: Math.floor((clampedFloor - 1) / exports.CAMPAIGN_REGION_FLOOR_COUNT) + 1,
        regionId,
...
function buildCampaignRegionZoneName(floorOrdinal, pick) {
