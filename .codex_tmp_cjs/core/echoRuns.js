...
exports.summarizeEchoEncounter = summarizeEchoEncounter;
exports.getActiveEchoRun = getActiveEchoRun;
exports.hasActiveEchoRun = hasActiveEchoRun;
exports.clearActiveEchoRun = clearActiveEchoRun;
exports.isEchoBattle = isEchoBattle;
exports.getEchoDraftChoices = getEchoDraftChoices;
exports.startEchoRunSession = startEchoRunSession;
exports.rerollActiveEchoChoices = rerollActiveEchoChoices;
exports.applyEchoDraftChoice = applyEchoDraftChoice;
exports.selectEchoMapNode = selectEchoMapNode;
exports.getEchoModifierHardpoints = getEchoModifierHardpoints;
exports.launchActiveEchoEncounterBattle = launchActiveEchoEncounterBattle;
exports.commitEchoEncounterVictory = commitEchoEncounterVictory;
exports.finalizeEchoRunFromBattleDefeat = finalizeEchoRunFromBattleDefeat;
exports.abandonActiveEchoRun = abandonActiveEchoRun;
exports.getEchoResultsSummary = getEchoResultsSummary;
exports.getEchoModifierDef = getEchoModifierDef;
const gameStore_1 = require("../state/gameStore");
const saveSystem_1 = require("./saveSystem");
...
const ECHO_SQUAD_SCOPE_MODS = (0, fieldModDefinitions_1.getAllFieldModDefs)().filter((mod) => mod.scope === "squad");
...
function getDraftClassWeights(run, encounterDepth) {
    ...
    const affinityTotals = (0, affinity_1.createDefaultAffinities)();
...
function createAffinityBiasForClass(unitClass, encounterDepth, rng) {
    const affinities = (0, affinity_1.createDefaultAffinities)();
...
function getEquipmentPools() {
    const allEquipment = Object.values((0, equipment_1.getAllStarterEquipment)());
    return {
...
function rollEchoLoadout(unitClass, encounterDepth, rng) {
    ...
    const allowedWeapons = equipmentPools.weapons.filter((equipment) => (equipment.slot === "weapon" && (0, equipment_1.canEquipWeapon)(unitClass, equipment.weaponType)));
...
function createEchoStats(unitClass, encounterDepth, rng) {
    const classDef = (0, classes_1.getClassDefinition)(unitClass);
...
function createEchoUnitDraftOption(run, optionIndex, encounterDepth, seedKey) {
    ...
    const pwr = (0, pwr_1.calculatePWR)({ unit: runtimeUnit });
    return {
        ...
        pwr,
        pwrBand: (0, pwr_1.getPWRBand)(pwr),
...
function getFieldLaneChoice(run, rewardSeed) {
    ...
    const missingFields = (0, echoFieldEffects_1.getEchoFieldCatalog)().filter((field) => !run.fields.some((owned) => owned.id === field.id));
    if (missingFields.length > 0) {
        const baseChoice = pickOne(rng, missingFields);
        const choiceField = (0, echoFieldEffects_1.buildEchoFieldDefinition)(baseChoice.id, `echo_field_${run.id}_${rewardSeed}`, 1);
...
function createMilestoneChoices(run, nodeId) {
    ...
    return [
        ...
        (run.fields.length < (0, echoFieldEffects_1.getEchoFieldCatalog)().length || run.fields.some((field) => field.level < MAX_FIELD_LEVEL))
...
function generateInitialFieldChoices(runId) {
    ...
    return pickUnique(rng, (0, echoFieldEffects_1.getEchoFieldCatalog)(), FIELD_DRAFT_CHOICE_COUNT).map((field, index) => createFieldChoice((0, echoFieldEffects_1.buildEchoFieldDefinition)(field.id, `echo_field_${runId}_${index}`, 1), `echo_initial_field_choice_${index}`, "field_draft"));
}
...
function readEchoRun() {
    return (0, gameStore_1.getGameState)().echoRun ?? null;
}
function persistEchoRun(run, extras) {
    ...
    if (extras?.autosave !== false) {
        void (0, saveSystem_1.triggerAutosave)((0, gameStore_1.getGameState)());
    }
    return run;
}
...
function summarizeEchoEncounter(battle) {
...
function hasActiveEchoRun() {
    return readEchoRun() !== null;
}
function clearActiveEchoRun() {
    (0, gameStore_1.updateGameState)((prev) => ({
        ...prev,
        echoRun: null,
    }));
...
function getEchoDraftChoices() {
    return readEchoRun()?.draftChoices ?? [];
}
function startEchoRunSession() {
    (0, saveSystem_1.enableAutosave)(() => (0, gameStore_1.getGameState)());
...
function rerollActiveEchoChoices() {
    const run = readEchoRun();
    ...
    if (choice.optionType === "field_upgrade" && choice.fieldDefinition) {
        return {
            ...
            fields: run.fields.map((field) => (field.draftId === choice.fieldDefinition?.draftId
                ? (0, echoFieldEffects_1.buildEchoFieldDefinition)(field.id, field.draftId, Math.min(MAX_FIELD_LEVEL, field.level + 1))
                : field)),
...
function applyEchoDraftChoice(choiceId) {
    const run = readEchoRun();
    if (!run) {
        return null;
    }
...
function selectEchoMapNode(nodeId) {
    const run = readEchoRun();
...
function launchActiveEchoEncounterBattle() {
    const run = readEchoRun();
    if (!run) {
        return null;
    }
    ...
    const battle = (0, battleFromEncounter_1.createBattleFromEncounter)(state, encounter, `${run.seed}:battle:${node.id}:${run.encounterNumber}`, {
...
function commitEchoEncounterVictory(battle) {
    const run = readEchoRun();
    if (!run) {
        return null;
    }
...
function finalizeEchoRunFromBattleDefeat(battle) {
    const run = readEchoRun();
    if (!run) {
        return null;
    }
...
function getEchoResultsSummary() {
    const run = readEchoRun();
    return run?.resultsSummary ?? (run ? buildEchoScoreSummary(run) : null);
}
function getEchoModifierDef(modifierId) {
    if (!modifierId) {
        return null;
    }
    return (0, fieldModDefinitions_1.getFieldModDef)(modifierId) ?? null;
}
