...
function createBattleFromEncounter(gameState, encounter, encounterSeed, options = {}) {
    // Get party units
    ...
    const units = {};
    const activeRunMealBuff = (0, tavernMeals_1.getActiveRunTavernMealBuff)(gameState);
    ...
    partyUnitIds.forEach(unitId => {
        ...
        if (baseUnit) {
            units[unitId] = (0, battle_1.createBattleUnitState)(baseUnit, {
                isEnemy: false,
    ...
    encounter.enemyUnits.forEach(({ enemyId, count, levelMod = 0, elite = false }) => {
        const importedTemplate = (0, technica_1.getImportedUnit)(enemyId);
        if (importedTemplate?.spawnRole === "enemy") {
            for (let i = 0; i < count; i++) {
                ...
                positionIndex++;
                units[instanceId] = (0, battle_1.createBattleUnitState)(createImportedEnemyBaseUnit(instanceId, importedTemplate, pos), {
                    isEnemy: true,
                    pos,
                }, gameState.equipmentById);
            }
        ...
        const enemyDef = (0, enemies_1.getEnemyDefinition)(enemyId);
        if (!enemyDef) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            units[instanceId] = (0, battle_1.createBattleUnitState)(baseUnit, {
                isEnemy: true,
                pos: pos,
            }, gameState.equipmentById);
        }
    ...
    const boardLayout = (0, terrainGeneration_1.generateStructuredBoardLayout)(encounter.gridWidth, encounter.gridHeight, seedForCover, "encounter");
    // Create tiles
    const tiles = [];
    ...
    const tilesWithCover = (0, coverGenerator_1.generateCover)(tiles, encounter.gridWidth, encounter.gridHeight, seedForCover, reservedCells);
