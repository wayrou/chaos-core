...
exports.PAK_DATABASE = exports.LIBRARY_CARD_DATABASE = void 0;
exports.upsertLibraryCard = upsertLibraryCard;
exports.getLibraryCardDatabase = getLibraryCardDatabase;
exports.getLibraryCard = getLibraryCard;
exports.getDefaultGearSlots = getDefaultGearSlots;
exports.addCardsToLibrary = addCardsToLibrary;
exports.hasCard = hasCard;
exports.getLibraryCards = getLibraryCards;
exports.filterLibraryCards = filterLibraryCards;
exports.slotCard = slotCard;
exports.unslotCard = unslotCard;
exports.getGearCards = getGearCards;
exports.compileDeck = compileDeck;
exports.getDeckPreview = getDeckPreview;
exports.openPAK = openPAK;
exports.generateBattleRewardCards = generateBattleRewardCards;
exports.getStarterCardLibrary = getStarterCardLibrary;
// ----------------------------------------------------------------------------
// TYPES
// ----------------------------------------------------------------------------
...
function upsertLibraryCard(card) {
    exports.LIBRARY_CARD_DATABASE[card.id] = card;
}
function getLibraryCardDatabase() {
    const importedCards = Object.fromEntries((0, technica_1.getAllImportedCards)().map((card) => [card.id, toLibraryCardFromImportedCard(card)]));
    return {
        ...exports.LIBRARY_CARD_DATABASE,
        ...importedCards,
    };
...
function getDefaultGearSlots(equipmentId, equipment) {
    ...
    if (equipment?.chassisId) {
        const chassis = (0, gearCatalog_1.getChassisById)(equipment.chassisId);
        if (chassis) {
 ...
 */
function slotCard(gearSlots, cardId) {
 ...
 */
function unslotCard(gearSlots, slotIndex) {
 ...
 */
function getDeckPreview(compiled) {
    const cardDatabase = getLibraryCardDatabase();
 ...
 */
function openPAK(pakId) {
    const pak = exports.PAK_DATABASE[pakId];
    if (!pak)
        return [];
    const cards = [];
...
function getStarterCardLibrary() {
    ...
    (0, technica_1.getAllImportedCards)().forEach((card) => {
        starterLibrary[card.id] = Math.max(starterLibrary[card.id] ?? 0, 1);
    });
    return Object.fromEntries(Object.entries(starterLibrary).filter(([cardId]) => !(0, technica_2.isTechnicaContentDisabled)("card", cardId)));
}
