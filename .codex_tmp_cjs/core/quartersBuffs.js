...
exports.getQuartersBuffsState = getQuartersBuffsState;
exports.rollRandomBuff = rollRandomBuff;
exports.restAtBunk = restAtBunk;
exports.canRest = canRest;
exports.consumeBuffOnRunStart = consumeBuffOnRunStart;
exports.getBuffDefinition = getBuffDefinition;
 ...
 */
function getQuartersBuffsState(state) {
 ...
 */
async function restAtBunk() {
    const { getGameState, updateGameState } = await Promise.resolve().then(() => require("../state/gameStore"));
 ...
 */
function canRest(state) {
    const buffsState = getQuartersBuffsState(state);
    const visitIndex = state.baseCampVisitIndex ?? 0;
    // Can rest if no buff, or buff is from previous visit, or buff is consumed
    if (!buffsState.currentBuff)
        return true;
    if (buffsState.currentBuff.consumed)
        return true;
    if (buffsState.lastRestVisitIndex !== visitIndex)
        return true;
    return false;
}
/**
 ...
 */
async function consumeBuffOnRunStart() {
    const { getGameState, updateGameState } = await Promise.resolve().then(() => require("../state/gameStore"));
    const state = getGameState();
    const buffsState = getQuartersBuffsState(state);
