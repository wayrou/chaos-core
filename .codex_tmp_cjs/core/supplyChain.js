...
exports.getSupplyState = getSupplyState;
exports.updateSupplyState = updateSupplyState;
exports.getEdgeId = getEdgeId;
exports.computeSupplyFlow = computeSupplyFlow;
exports.computeSupplyHealth = computeSupplyHealth;
exports.rollLinkAttacks = rollLinkAttacks;
exports.applyLinkAttack = applyLinkAttack;
exports.advanceSupplyStep = advanceSupplyStep;
 ...
 */
function getSupplyState(state) {
    const operation = (0, ops_1.getCurrentOperation)(state);
    if (!operation)
        return null;
    // Supply state is stored on the operation
    const supplyState = operation.supplyState;
    if (!supplyState) {
        ...
        const floor = (0, ops_1.getCurrentFloor)(operation);
        if (floor) {
 ...
 */
function updateSupplyState(state, updater) {
    const operation = (0, ops_1.getCurrentOperation)(state);
    if (!operation)
        return;
 ...
 */
function computeSupplyFlow(nodes, supplyState, profile) {
    const flowCache = {};
    // Build node map for quick lookup
    const nodeMap = new Map();
    const nodeDepth = new Map(); // Depth from start
    for (const node of nodes) {
    ...
    const startNode = nodes.find(n => {
        ...
        const hasIncoming = nodes.some(other => other.connections?.includes(n.id));
        return !hasIncoming;
    }) || nodes[0];
 ...
 */
function computeProfileWeights(edges, nodes, nodeDepth, profile) {
    if (edges.length === 0)
        return [];
    if (edges.length === 1)
        return [{ edgeId: edges[0].edgeId, weight: 1.0 }];
    const weights = [];
    for (const edge of edges) {
        ...
        switch (profile) {
            ...
            case "defensive_hold":
                ...
                const isKeyRoom = toNode.isKeyRoom === true;
 ...
 */
function computeSupplyHealth(flowCache, nodes) {
 ...
 */
function rollLinkAttacks(supplyState, operationSeed, floorIndex, nodes) {
    const attacks = [];
 ...
 */
function applyLinkAttack(supplyState, edgeId, damage) {
    const edgeState = supplyState.edgeState[edgeId] || { integrity: DEFAULT_EDGE_INTEGRITY };
 ...
 */
function advanceSupplyStep(state) {
    const operation = (0, ops_1.getCurrentOperation)(state);
    if (!operation)
        return;
    const floor = (0, ops_1.getCurrentFloor)(operation);
