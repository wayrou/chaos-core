...
exports.CODEX_DATABASE = void 0;
exports.syncImportedCodexUnlocks = syncImportedCodexUnlocks;
exports.getCodexDatabase = getCodexDatabase;
exports.unlockCodexEntry = unlockCodexEntry;
exports.getUnlockedCodexEntries = getUnlockedCodexEntries;
exports.isCodexEntryUnlocked = isCodexEntryUnlocked;
exports.debugUnlockAllCodexEntries = debugUnlockAllCodexEntries;
...
function getImportedCodexDatabase() {
    return (0, technica_1.getAllImportedCodexEntries)().map(normalizeImportedCodexEntry);
}
...
function getCodexDatabase() {
    const importedEntries = getImportedCodexDatabase();
    const importedIds = new Set(importedEntries.map((entry) => entry.id));
    return [...exports.CODEX_DATABASE.filter((entry) => !importedIds.has(entry.id)), ...importedEntries];
}
/**
 * Unlock a codex entry so it's readable to the player forever
 */
function unlockCodexEntry(entryId) {
    const state = (0, gameStore_1.getGameState)();
    if (!state.unlockedCodexEntries) {
        (0, gameStore_1.updateGameState)((s) => ({ ...s, unlockedCodexEntries: [entryId] }));
 ...
 */
function getUnlockedCodexEntries() {
