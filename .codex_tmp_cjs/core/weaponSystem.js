...
exports.NODE_DAMAGE_EFFECTS = exports.WEAPON_NODE_NAMES = void 0;
exports.getWeaponClutches = getWeaponClutches;
exports.getWeaponHeatProfile = getWeaponHeatProfile;
exports.getWeaponAmmoProfile = getWeaponAmmoProfile;
exports.createWeaponRuntimeState = createWeaponRuntimeState;
exports.hasActiveClutch = hasActiveClutch;
exports.activateClutch = activateClutch;
exports.deactivateClutch = deactivateClutch;
exports.activateDoubleClutch = activateDoubleClutch;
exports.deactivateDoubleClutch = deactivateDoubleClutch;
exports.toggleWeaponClutch = toggleWeaponClutch;
exports.resetClutches = resetClutches;
exports.commitClutchWear = commitClutchWear;
exports.getWearPenalties = getWearPenalties;
exports.getEffectiveMaxHeat = getEffectiveMaxHeat;
exports.getHeatZoneProfile = getHeatZoneProfile;
exports.getHeatZone = getHeatZone;
exports.getHeatZoneColor = getHeatZoneColor;
exports.getAccuracyPenalty = getAccuracyPenalty;
exports.getDamagePenalty = getDamagePenalty;
exports.getWeaponCardModifierSnapshot = getWeaponCardModifierSnapshot;
exports.canUseOverwatch = canUseOverwatch;
exports.canUseMultiAttack = canUseMultiAttack;
exports.checkWeaponJam = checkWeaponJam;
exports.getExtraStrainCost = getExtraStrainCost;
exports.isWeaponDestroyed = isWeaponDestroyed;
exports.getWeaponCardAmmoCost = getWeaponCardAmmoCost;
exports.getWeaponCardHeatDelta = getWeaponCardHeatDelta;
exports.getWeaponCardBlockReason = getWeaponCardBlockReason;
exports.getWeaponActionDisabledReason = getWeaponActionDisabledReason;
exports.addHeat = addHeat;
exports.removeHeat = removeHeat;
exports.passiveCooling = passiveCooling;
exports.useAmmo = useAmmo;
exports.quickReload = quickReload;
exports.fullReload = fullReload;
exports.fieldPatch = fieldPatch;
exports.ventWeapon = ventWeapon;
exports.rollWeaponHit = rollWeaponHit;
exports.rollWeaponNodeHit = rollWeaponNodeHit;
exports.damageNode = damageNode;
exports.repairNode = repairNode;
exports.markWeaponCardPlayed = markWeaponCardPlayed;
exports.consumeQueuedModifier = consumeQueuedModifier;
exports.advanceWeaponTurn = advanceWeaponTurn;
exports.triggerWeaponOverheat = triggerWeaponOverheat;
...
function getWeaponClutches(weapon) {
...
function getWeaponHeatProfile(weapon) {
...
function getWeaponAmmoProfile(weapon) {
...
function hasActiveClutch(state) {
    return state.activeClutchIds.length > 0;
}
function activateClutch(state, weapon) {
...
function deactivateClutch(state, weapon) {
    const clutchId = weapon ? getWeaponClutches(weapon)[0]?.id : "primary_clutch";
    if (!clutchId) {
        return syncLegacyClutchFlags(state, weapon);
    }
...
function activateDoubleClutch(state, weapon) {
...
function deactivateDoubleClutch(state, weapon) {
    const clutchId = weapon ? getWeaponClutches(weapon)[1]?.id : "secondary_clutch";
    if (!clutchId) {
        return syncLegacyClutchFlags(state, weapon);
    }
...
function toggleWeaponClutch(state, weapon, clutchId) {
...
function resetClutches(state, weapon) {
    return syncLegacyClutchFlags({
        ...state,
        activeClutchIds: [],
    }, weapon);
}
function commitClutchWear(state) {
    return state;
}
function getWearPenalties(wear) {
    if (wear <= 0) {
...
function getEffectiveMaxHeat(state, weapon) {
...
function getHeatZoneProfile(state, weapon) {
    const heatProfile = getWeaponHeatProfile(weapon);
    if (!heatProfile) {
        return null;
    }
...
function getHeatZone(state, weapon) {
...
function getHeatZoneColor(zone) {
    switch (zone) {
        case "stable":
            return "#4ade80";
        case "warning":
...
function getAccuracyPenalty(state) {
...
function getDamagePenalty(state) {
...
function getWeaponCardModifierSnapshot(state, weapon, cardRules) {
...
function canUseOverwatch(state) {
    return state.nodes[1] !== "broken" && state.nodes[1] !== "destroyed";
}
function canUseMultiAttack(state) {
    return state.nodes[3] === "ok";
}
function checkWeaponJam(state) {
    return state.nodes[3] === "broken" && Math.random() < 0.33;
}
function getExtraStrainCost(state, isFirstWeaponCardThisTurn) {
...
function isWeaponDestroyed(state) {
    return Object.values(state.nodes).some((status) => status === "destroyed");
}
function getWeaponCardAmmoCost(state, weapon, cardRules) {
    const ammoProfile = getWeaponAmmoProfile(weapon);
    if (!ammoProfile) {
        return 0;
    }
...
function getWeaponCardHeatDelta(state, weapon, cardRules) {
    const heatProfile = getWeaponHeatProfile(weapon);
    if (!heatProfile) {
        return 0;
    }
...
function getWeaponCardBlockReason(state, weapon, cardRules) {
    if (!cardRules) {
        return null;
    }
...
function getWeaponActionDisabledReason(action, state, weapon) {
    const ammoProfile = getWeaponAmmoProfile(weapon);
    const heatProfile = getWeaponHeatProfile(weapon);
...
function addHeat(state, weapon, amount) {
    const heatProfile = getWeaponHeatProfile(weapon);
...
function removeHeat(state, amount, options = {}) {
    if (amount <= 0) {
        return state;
    }
    const next = cloneState(state);
...
function passiveCooling(state, weapon) {
    if (!weapon) {
...
function useAmmo(state, weapon, amount = 1) {
    const ammoProfile = getWeaponAmmoProfile(weapon);
...
function quickReload(state, weapon) {
    const ammoProfile = getWeaponAmmoProfile(weapon);
    if (!ammoProfile) {
...
function fullReload(state, weapon) {
    const ammoProfile = getWeaponAmmoProfile(weapon);
    if (!ammoProfile) {
        return { state, strainCost: 0 };
    }
...
function ventWeapon(state, _weapon) {
    let next = cloneState(state);
    next.currentHeat = 0;
...
function repairNode(state, nodeId) {
    const next = cloneState(state);
    const currentStatus = next.nodes[nodeId];
...
function markWeaponCardPlayed(state, weapon, cardRules) {
    const next = cloneState(state);
...
function consumeQueuedModifier(state) {
    return {
        ...state,
        queuedModifier: { ...EMPTY_QUEUED_MODIFIER_STATE },
    };
}
function advanceWeaponTurn(state, weapon) {
    let next = cloneState(state);
...
function triggerWeaponOverheat(state, weapon) {
    const heatProfile = getWeaponHeatProfile(weapon);
    if (!heatProfile) {
