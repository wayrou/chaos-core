...
exports.createTrainingEncounter = createTrainingEncounter;
 ...
 */
function createTrainingEncounter(state, config) {
    ...
    encounter.isTraining = true;
    encounter.trainingConfig = config;
    return encounter;
}
/**
 * Generate enemy units for training based on difficulty
 */
function generateTrainingEnemies(config) {
    ...
    while (remaining > 0 && enemyPool.length > 0) {
        ...
        try {
            const enemyDef = (0, enemies_1.getEnemyDefinition)(randomEnemy);
            if (enemyDef) {
