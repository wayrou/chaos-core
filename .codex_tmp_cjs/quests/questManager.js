...
exports.getQuestAcceptanceFailureMessage = getQuestAcceptanceFailureMessage;
exports.initializeQuestState = initializeQuestState;
exports.replenishQuests = replenishQuests;
exports.syncQuestProgressInStore = syncQuestProgressInStore;
exports.acceptQuest = acceptQuest;
exports.updateQuestProgress = updateQuestProgress;
exports.failQuest = failQuest;
...
exports.getTotalQuestsCompleted = getTotalQuestsCompleted;
exports.generateNewQuest = generateNewQuest;
exports.abandonQuest = abandonQuest;
exports.completeNpcTurnInQuests = completeNpcTurnInQuests;
...
function getQuestRequirementFailure(quest, state = (0, gameStore_1.getGameState)()) {
    ...
    const missingKeyItemIds = requiredKeyItemIds.filter((keyItemId) => (0, keyItems_1.getOwnedKeyItemQuantity)(state, keyItemId) <= 0);
...
function areQuestRequirementsMet(quest, state = (0, gameStore_1.getGameState)()) {
    return !getQuestRequirementFailure(quest, state);
}
function getQuestAcceptanceFailure(questId, state = (0, gameStore_1.getGameState)()) {
    const quest = (0, questData_1.getQuestById)(questId);
    if (!quest) {
...
function getQuestAcceptanceFailureMessage(questId) {
...
function addGeneratedQuestsToState(state, count) {
    ...
    const newQuests = (0, questGenerator_1.generateRandomQuests)(resolvedCount);
    return {
        ...state,
        quests: {
            ...questState,
...
function settleQuestState(state, replenishGenerated = false) {
    const syncedState = (0, questRuntime_1.syncQuestProgressFromSnapshotState)(state);
    ...
    if (!state.quests) {
        const seededState = settleQuestState({
            ...
            quests: {
                ...
                activeQuests: (0, questGenerator_1.generateRandomQuests)(INITIAL_GENERATED_QUESTS),
                generatedQuestCount: INITIAL_GENERATED_QUESTS,
            },
        });
        (0, gameStore_1.updateGameState)(() => seededState);
        console.log(`[QUEST] Initialized quest state with ${seededState.quests?.activeQuests.length ?? 0} theater-aligned generated quests`);
        return;
    }
    (0, gameStore_1.updateGameState)((current) => settleQuestState(current, true));
}
function replenishQuests() {
    (0, gameStore_1.updateGameState)((state) => settleQuestState(state, true));
}
function syncQuestProgressInStore() {
    (0, gameStore_1.updateGameState)((state) => settleQuestState(state, false));
}
function acceptQuest(questId) {
    ...
    (0, gameStore_1.updateGameState)((state) => settleQuestState({
        ...state,
        quests: {
            ...(state.quests ?? createEmptyQuestState()),
            activeQuests: [...(state.quests?.activeQuests ?? []), (0, questData_1.cloneQuest)(quest)],
        },
    }));
    console.log(`[QUEST] Accepted quest: ${quest.title}`);
    return true;
}
function updateQuestProgress(objectiveType, target, amount = 1) {
...
function getAvailableQuests() {
    const state = (0, gameStore_1.getGameState)();
    const questState = getQuestState();
    return (0, questData_1.getAvailableQuests)().filter((quest) => !questState.completedQuests.includes(quest.id)
        && !questState.activeQuests.some((activeQuest) => activeQuest.id === quest.id)).filter((quest) => areQuestRequirementsMet(quest, state));
}
function getActiveQuest(questId) {
...
function generateNewQuest() {
    const quest = (0, questGenerator_1.generateRandomQuest)();
    (0, gameStore_1.updateGameState)((state) => ({
        ...state,
        quests: {
            ...(state.quests ?? createEmptyQuestState()),
...
function completeNpcTurnInQuests(npcId) {
    ...
    (0, gameStore_1.updateGameState)((state) => {
        ...
        questState.activeQuests.forEach((quest) => {
            ...
            if ((0, keyItems_1.getOwnedKeyItemQuantity)(nextState, turnIn.keyItemId) < turnIn.quantity) {
                remainingActiveQuests.push(quest);
                return;
            }
            const consumedResult = (0, keyItems_1.consumeKeyItemFromState)(nextState, turnIn.keyItemId, turnIn.quantity);
            ...
            completedCount += 1;
            nextState = (0, questRewards_1.applyQuestRewardsToState)(consumedResult.state, quest);
        });
        if (completedCount === 0) {
            return state;
        }
